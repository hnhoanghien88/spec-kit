...
**Cập nhật (spec Session Update 19 — hợp nhất hoàn toàn Add/Edit vào một popup; đơn giản hóa cột
Conditions; xóa hoàn toàn trang Add cũ + popup Assign condition, FR-001 đến FR-042)**: Thay đổi
**phạm vi lớn nhất** kể từ khi feature được tạo — đảo ngược có chủ đích quyết định "giữ dead code" của
Update 15 (research Quyết định 45/53). Toàn bộ luồng **Add cũ** (trang riêng `/eutr/documents/add`,
Screen1/List PO, Screen2, popup "Assign condition" cả 2 mode) và **Edit cũ** (popup đơn giản
`EutrDocumentsModal.jsx`, và việc Edit rẽ nhánh mở lại popup Assign condition cho Type="Upload
manual") bị **xóa hoàn toàn** khỏi codebase — không giữ làm dead code như Update 15, vì spec dùng từ
ngữ "loại bỏ hoàn toàn khỏi phạm vi feature" và các file này phụ thuộc lẫn nhau + phụ thuộc các
endpoint bị xóa cùng đợt nên không còn tự chạy được (khác hẳn trang Add cũ ở Update 15, vẫn tự đủ và
truy cập được qua URL trực tiếp).

Từ nay, **Add** và **Edit** dùng chung đúng **một** component popup — đổi tên
`EutrDocumentsAddDialog.jsx` (Update 15-18) → `EutrDocumentsFormDialog.jsx`, thêm prop `mode: 'add' |
'edit'` + `initialData` (research Quyết định 53). Mode `add` giữ nguyên hành vi Type/Step/Value-
chip/Upload đã có từ Update 15-18, **cộng thêm 2 trường ngày mới** Valid from (mặc định hôm nay) và
Valid to (mặc định `9999-12-31`), cả hai editable trước khi Upload và validate `validFrom ≤ validTo`
phía client (FR-014 đến FR-016) — giá trị này được truyền xuống 2 request upload hiện có
(`EutrMultiUploadFileRequest`/`EutrTypeMultiUploadFileRequest`, thêm field nullable `ValidFrom`/
`ValidTo`), `EutrUploadService` dùng `request.ValidFrom ?? DateTime.Today`/`request.ValidTo ??
MaxValidTo` thay 2 hằng số cố định trước đây (research Quyết định 55). Mode `edit` khóa Type
(disabled), hiển thị chip Value chỉ đọc (không control thêm/xóa), ẩn hẳn `EutrAddValueAutocomplete`,
thay nút Upload bằng Save; Step hiển thị/sửa được **cho mọi Type kể cả PO** (khác mode `add`, nơi PO
ẩn Step — quy tắc khác nhau có chủ đích giữa 2 mode, research Quyết định 60), chỉ ẩn khi document
không có bất kỳ `eutr_references` nào; dữ liệu nạp popup lấy thẳng từ `row` grid đã có sẵn (0 round-
trip HTTP mới, research Quyết định 59). Save gọi tuần tự `PUT /api/eutr-documents/{id}` (Valid from/
to) rồi — nếu Step hiển thị — `PUT /api/eutr-documents/{id}/step` (nếu có).

Backend: `PUT /api/eutr-documents/{id}/step` (route giữ nguyên) đổi hoàn toàn hành vi — từ "chỉ áp
dụng Type=PO, xóa-toàn-bộ-tạo-lại-1-dòng" sang `UPDATE eutr_references SET StepId=@stepId WHERE
DocumentId=@id` áp dụng cho **mọi Type**, giữ nguyên `RefValue`/`RefType`/số lượng bản ghi (khớp
FR-033) — logic chuyển từ `IEutrConditionAssignmentService` (bị xóa) sang method mới
`EutrDocumentsService.UpdateReferenceStepAsync`, request DTO đổi tên `EutrUpdatePoStepRequestDto` →
`EutrUpdateReferenceStepRequestDto` (research Quyết định 56). Cột **Conditions** đổi nguồn hoàn toàn:
từ nhóm `eutr_reference_details` theo `ConditionType` (Update 11) sang **flat, distinct `RefValue`**
của mọi `eutr_references` thuộc document — mở rộng SQL có sẵn của
`GetStepInfoByDocumentIdsAsync` (Update 8/14) thêm `r.RefValue`, `EutrDocumentsResponseDto.Conditions`
đổi kiểu `List<ConditionGroupDto>` → `List<string>` (research Quyết định 54) — áp dụng cho **mọi**
Type (không còn phân biệt PO/Upload manual/khác như trước).

Xóa hoàn toàn (research Quyết định 57): 5 endpoint (`get-unassigned`, `assign-conditions`,
`condition-assignment` GET/PUT, `list-po-references`) + `eutr-upload-manual-multi`
(`SharePointController`); service `IEutrConditionAssignmentService`/`EutrConditionAssignmentService`;
entity/repository `EutrReferenceDetails`/`IEutrReferenceDetailsRepository`/
`EutrReferenceDetailsRepository`; mọi DTO/projection chỉ phục vụ các luồng trên
(`EutrManualMultiUploadFileRequest`, `EutrAssignConditionsRequestDto`, `EutrConditionRowDto`,
`EutrUpdateConditionAssignmentRequestDto`, `EutrDocumentConditionAssignmentDto`, `ConditionGroupDto`,
`EutrConditionGroupRow`, `EutrDocumentsListPoReferencesRequestDto`, `EutrDocumentsPoReferenceDto`,
`EutrDocumentsPoReferenceItemDto`, `EutrReferencePoDocumentInfo`); `EutrDocumentsService.
GetUnassignedPagedAsync`/`GetPoReferencesAsync`; `IEutrReferencesRepository.
GetDocumentsByPoCodesAsync`/`GetUnassignedDocumentsPagedAsync`; `EutrUploadService.
UploadManualMultipleToSharePointAndSaveDataAsync`. **Ngoại lệ giữ nguyên**: SQL 2 bước của
`EutrReferencesRepository.DeleteByDocumentIdAsync` (dọn `eutr_reference_details` mồ côi trước khi xóa
`eutr_references`, Update 11) — bảng `eutr_reference_details` KHÔNG bị xóa/migrate, dữ liệu lịch sử
vẫn có thể vi phạm khóa ngoại nếu bỏ bước này (research Quyết định 58). Frontend xóa tương ứng:
`EutrDocumentsAdd.jsx` + route `/eutr/documents/add`, `EutrDocumentsModal.jsx`,
`AssignConditionDialog.jsx`, cùng mọi use case/method chỉ dùng bởi các file này. **Không migration DB
mới** (mọi cột liên quan — `RefValue`, `StepId`, `RefType`, `ValidFrom`/`ValidTo` — đã tồn tại từ
Update 7/14).

**Cập nhật (spec Session Update 20 — lọc Step theo Assign Steps của Type; mặc định chọn dòng đầu,
FR-043 đến FR-045)**: Combobox **Step** trong popup `EutrDocumentsFormDialog.jsx` (cả mode Add và
Edit, khi Type khác "PO") không còn nạp **toàn bộ** `eutr_steps` — đổi sang lọc theo dữ liệu
**Assign Steps** đã có sẵn từ feature `006-eutr-reference-types`: gọi lại use case **đã tồn tại sẵn**
`GetByTypeIdEutrReferenceTypeDetailsUseCase` (repository `repositories.eutrReferenceTypeDetails`,
endpoint có sẵn `GET /api/eutr-reference-type-details/by-type/{typeId}`, entity backend
`EutrReferenceTypeDetails`/repository `EutrReferenceTypeDetailsRepository` — toàn bộ hạ tầng này **đã
được xây dựng xong** ở feature `006`, không cần thêm dòng backend nào cho Update 20 này) mỗi khi Type
đổi (Add) hoặc khi mở popup Edit (Type đã khóa, dùng Type hiện tại của document). Danh sách Step hiển
thị = kết quả trả về (theo `stepId`), map sang hình dạng `{ id, name }` mà `Autocomplete` Step hiện có
đang kỳ vọng — ưu tiên đối chiếu với danh sách `eutr_steps` đầy đủ đã tải sẵn (`getEutrStepsUseCase`,
Update 15) để giữ đúng object reference, dùng thẳng `stepName` của response làm fallback nếu không
khớp. Mode Add: chọn sẵn dòng đầu tiên của danh sách lọc làm mặc định (`setStep(filteredSteps[0] ??
null)`) ngay sau khi tải xong; đổi Type tải lại danh sách và mặc định lại dòng đầu. Mode Edit: Step
hiện tại của document (nạp từ `initialData`) luôn được đảm bảo có mặt trong danh sách hiển thị — nếu
không nằm trong kết quả lọc (đã bị gỡ khỏi Assign Steps sau khi document được tạo), component chèn
thêm chính Step đó vào đầu danh sách thay vì loại bỏ, và **không** ép chọn lại mặc định (giữ nguyên
Step đang hiển thị). Type = "PO" không đổi (Step tiếp tục ẩn hoàn toàn). **0 thay đổi backend** — toàn
bộ hạ tầng đọc (entity/repository/controller/policy) đã tồn tại sẵn từ feature `006-eutr-reference-
types`; chỉ sửa đúng 1 file frontend hiện có (`EutrDocumentsFormDialog.jsx`) để wiring lại use case đã
có sẵn.

**Cập nhật (spec Session Update 21 — thêm search box lọc danh sách theo Type/Step name/Conditions,
FR-046 đến FR-050)**: Màn hình danh sách chính bổ sung một **search box** phía trên bảng (dropdown
**Type**, dropdown **Step name**, ô nhập **Conditions**, nút **Search**). Backend: **không thêm
endpoint mới** — mở rộng hành vi nội bộ của `EutrDocumentsService.GetPagedAsync` (endpoint #2,
`POST /api/eutr-documents/get-all` — không đổi path/policy/request shape công khai) để tiêu thụ **3
"cột lọc ảo"** không tồn tại trên entity `EutrDocuments`: `TypeId`, `StepId`, `Conditions`. Trước khi
gọi `base.GetPagedAsync`, service tách 3 filter này (nếu có) ra khỏi `request.Filters` (generic
repository sẽ tự bỏ qua các `Column` không khớp property nào trên `TEntity`, nên phải tách thủ công
trước khi build query — research Quyết định 62); nếu có ít nhất 1 trong 3, gọi method mới
`IEutrReferencesRepository.GetMatchingDocumentIdsAsync(long? typeId, long? stepId, string?
conditionsQuery, ct)` (SQL 3 `EXISTS` độc lập trên `eutr_references`, mỗi điều kiện có thể khớp một
dòng khác nhau của cùng document — đúng quyết định đã chốt ở spec Update 21, research Quyết định 63)
để lấy danh sách `DocumentId` khớp; danh sách rỗng → trả về `PagedResult` rỗng ngay (bỏ qua
`base.GetPagedAsync`); ngược lại, thêm 1 `FilterRequest { Column = "Id", Operator = "in", Value =
string.Join(",", matchingIds) }` vào phần filter còn lại rồi gọi `base.GetPagedAsync` như bình
thường — tái sử dụng **nguyên vẹn** cơ chế `Operator = "in"` đã có sẵn trong `Shared.Dapper`
generic repository (đã dùng bởi filter `Id` — cột này đã nằm trong whitelist entity — và có tiền lệ
"tính id ở nơi khác rồi IN-filter query cha" tại `AllCompliancesController`/`ViewCompliancesController`,
xem research Quyết định 62). **Không migration DB mới, không entity/DTO/endpoint mới** — chỉ 1
method mới trên `IEutrReferencesRepository`/`EutrReferencesRepository` đã có sẵn từ Update 8.
Frontend: clone component **`ComplianceFilterBar.jsx`** (`presentation/components/`, đã dùng bởi
`compliance-master`/`compliance-management`) và wiring pattern `handleSearch`/`searchFilters`/
`defaultFilters` của `compliance-master/index.jsx` — tạo component nhỏ mới
`EutrDocumentsFilterBar.jsx` (chỉ 3 control: Autocomplete Type dùng lại `GetEutrReferenceTypesUseCase`
đã có từ popup Add; Autocomplete Step name dùng lại `GetEutrStepsUseCase` đã có, tải **toàn bộ**
`eutr_steps` không lọc theo Type — khác cơ chế Assign Steps của Update 20, đúng quyết định đã chốt ở
spec; TextField Conditions; nút Search), sửa `useEutrDocumentsData.js` nhận thêm tham số
`defaultFilters = []` (mẫu `useComplianceMasterData`) gộp vào `filterPayload` trước khi gọi
`getPagingEutrDocumentsUseCase.execute(...)`, và `index.jsx` thêm state `searchFilters` + hàm
`handleSearch` (build `filters` từ Type/Step/Conditions đang chọn, `setSearchFilters`, reset
`paginationModel.page` về 0) — clone đúng cấu trúc đã có ở `compliance-master/index.jsx` (research
Quyết định 64). Không Clear button (spec chỉ yêu cầu đúng 4 control: Type/Conditions/step name/
Search) — xóa từng điều kiện thủ công rồi bấm Search lại để khôi phục danh sách đầy đủ.

**Cập nhật (spec Session Update 22 — Edit cho phép thêm/xóa chip Value khi Type khác "PO", FR-051
đến FR-055)**: Vùng chip **Value** trong `EutrDocumentsFormDialog.jsx` (mode `edit`) không còn **luôn**
chỉ đọc (Update 19) — chỉ còn chỉ đọc khi document có Type = "PO" (`isPoType`, cờ đã tính sẵn từ Add,
xem Update 15). Với Type khác "PO" (kể cả "Vendor"), dialog hiển thị lại component có sẵn
`EutrAddValueAutocomplete.jsx` (Update 15/16 — self-contained, không cần sửa gì bên trong: đã tự áp
dụng đúng gợi ý theo `type.name`/giới hạn 1 chip cho "Vendor"/dán nhiều giá trị) và bật lại `onDelete`
trên từng `Chip` hiện có — cả hai đổi từ điều kiện `!isEdit` sang `!isEdit || !isPoType`
(`showEditableChips = isEdit && initialData?.refType != null && !isPoType`, cùng cách tính với
`showStep` đã có). `canSubmit` (mode edit) thêm điều kiện `!showEditableChips || chips.length > 0`
(chặn Save khi xóa hết chip mà không thêm lại, spec kịch bản 17) — không cần logic "chặn thêm >1 chip
cho Vendor" mới, kế thừa miễn phí từ chính `EutrAddValueAutocomplete`.

Backend: **không endpoint mới** — mở rộng chính request/response của `PUT /api/eutr-documents/{id}/
step` (endpoint #12, Update 19) thay vì thêm route. `EutrUpdateReferenceStepRequestDto` thêm 1 field
nullable `List<string>? RefValues` — `null` (Type = "PO", hoặc mọi Edit trước Update 22) giữ nguyên
hành vi cũ (chỉ `UPDATE StepId`); có giá trị (Type khác "PO") kích hoạt bước đối chiếu (reconcile) mới
trong `EutrDocumentsService.UpdateReferenceStepAsync` — đọc lại tập `RefValue` hiện có của document
qua **method đọc đã có sẵn** `GetStepInfoByDocumentIdsAsync` (Update 8/14/19, KHÔNG thêm method đọc
mới), tính chênh lệch (`Except`, so sánh không phân biệt hoa/thường) để suy ra chip cần thêm/cần xóa,
rồi gọi 2 method **mới** trên `IEutrReferencesRepository`/`EutrReferencesRepository`:
`DeleteByDocumentIdAndRefValuesAsync` (raw SQL, dọn kèm `eutr_reference_details` mồ côi theo cùng mẫu
2 bước của `DeleteByDocumentIdAsync`, Update 11) và `AddReferencesAsync` (raw SQL INSERT, dùng lại
`RefType` đọc được từ dòng hiện có — không cần frontend gửi lại `TypeId`); toàn bộ (đọc, xóa, thêm,
cập nhật `StepId` bằng `UpdateStepIdByDocumentIdAsync` sẵn có) gộp trong 1 transaction mới (cùng mẫu
`_unitOfWork.BeginTransactionAsync`/`CommitAsync`/`RollbackAsync` đã dùng ở `DeleteAsync`/
`DeleteMultiAsync`, Update 9 — bản thân `UpdateReferenceStepAsync` trước Update 22 KHÔNG có transaction
vì chỉ 1 câu lệnh, nay cần vì có nhiều bước ghi). Cân nhắc và **loại bỏ** cách tiếp cận
"xóa toàn bộ rồi tạo lại toàn bộ" theo mẫu `ComplMasterConditionPersistenceService.ReplaceAsync` (đã
dùng ở Update 12 cho luồng Assign condition cũ) — vì cách đó làm mất `Id`/audit gốc của các dòng không
đổi, không khớp yêu cầu spec "chỉ thêm/xóa đúng phần chênh lệch" (research Quyết định 65-67).

Frontend: `toRefValue` (helper đã có sẵn trong `EutrDocumentsFormDialog.jsx`, dùng để build
`refValues` cho luồng Upload) được tái sử dụng để build payload `refValues` gửi cùng `stepId` khi
`showEditableChips` là true; API client `eutrDocumentsApi.updateReferenceStep`/
`RestEutrDocumentsRepository.updateReferenceStep`/`IEutrDocumentsRepository.updateReferenceStep`/
`UpdateEutrDocumentReferenceStepUseCase.execute` đều thêm tham số thứ 3 `refValues` (tùy chọn, `pass-
...
**Update 19**: KHÔNG migration DB mới — mọi cột cần (`RefValue`, `StepId`, `RefType` trên
`eutr_references`; `ValidFrom`/`ValidTo` trên `eutr_documents`) đã tồn tại từ Update 7/14. Bảng
`eutr_reference_details` (Update 11) KHÔNG bị xóa/migrate — chỉ không còn entity/repository backend
nào đọc/ghi nó (dữ liệu lịch sử giữ nguyên, khóa ngoại `eutr_reference_details_refid_foreign` vẫn
được tôn trọng qua SQL 2 bước có sẵn của `DeleteByDocumentIdAsync`, research Quyết định 58). Thay đổi
chủ yếu là **xóa code** (5 endpoint + 1 service + 1 entity/repository + nhiều DTO, xem Summary) và
**mở rộng 2 SQL/DTO đã có** (`GetStepInfoByDocumentIdsAsync` thêm `RefValue`; 2 request upload thêm
`ValidFrom`/`ValidTo`), không phải thêm hạ tầng DB mới nào.
**Update 20**: KHÔNG migration DB mới, KHÔNG entity/repository/endpoint backend mới — tái sử dụng
nguyên vẹn hạ tầng đọc bảng `eutr_reference_type_details` (entity `EutrReferenceTypeDetails`,
...
xây dựng đầy đủ bởi feature `006-eutr-reference-types` — feature `004-eutr-documents` chỉ tiêu thụ
(consume) read-only từ frontend (research Quyết định 61).
**Update 21**: KHÔNG migration DB mới, KHÔNG entity/DTO/endpoint mới — thêm đúng 1 method mới
`GetMatchingDocumentIdsAsync` trên `IEutrReferencesRepository`/`EutrReferencesRepository` đã có sẵn từ
Update 8 (SQL 3 `EXISTS` độc lập trên `eutr_references`, đọc-only); endpoint `get-all` hiện có mở rộng
hành vi nội bộ để tiêu thụ 3 "cột lọc ảo" `TypeId`/`StepId`/`Conditions` trong `request.Filters` rồi
tái dùng cơ chế `Operator = "in"` đã có sẵn của generic repository — không đổi path/policy/request
shape công khai của endpoint (research Quyết định 62/63).
...
thủ công (FR-075); với Type khác "PO", toàn bộ hành vi Update 15/16 giữ nguyên không đổi. **Update
19**: Add/Edit dùng chung 1 popup (`EutrDocumentsFormDialog.jsx`, FR-008/FR-026); popup Add thêm
Valid from/Valid to editable (mặc định hôm nay/`9999-12-31`), validate `validFrom ≤ validTo` chặn
Upload/Save (FR-014 đến FR-016); Edit khóa Type, chip chỉ đọc, Step khả dụng cho MỌI Type kể cả PO
(FR-027 đến FR-029, khác quy tắc ẩn Step của Add); Save Edit MUST chỉ đổi Step (áp dụng cho mọi bản
ghi `eutr_references` hiện có, giữ nguyên `RefValue`/`RefType`/số lượng) và/hoặc Valid from/to, không
thêm/xóa bản ghi nào (FR-033); cột Conditions đổi nguồn sang `RefValue` phẳng distinct của
`eutr_references`, bỏ hẳn `eutr_reference_details` (FR-005); trang Add cũ, popup Edit cũ, popup
Assign condition (cùng 5 endpoint + service liên quan) bị xóa hoàn toàn khỏi codebase. **Update 20**:
Combobox Step (Type khác "PO") trong popup Add/Edit MUST chỉ hiển thị Step có bản ghi
`eutr_reference_type_details` khớp `TypeId` đang chọn (FR-043); mặc định chọn dòng đầu danh sách đã
lọc ở mode Add (FR-044); mode Edit MUST đảm bảo Step hiện tại của document luôn hiển thị được dù
không còn trong danh sách đã lọc (FR-045). **Update 21**: Màn hình danh sách chính MUST hiển thị
search box (Type/Step name/Conditions/Search) phía trên bảng (FR-046); nhấn Search MUST lọc bảng theo
mọi điều kiện đã cung cấp, kết hợp AND, mỗi điều kiện chỉ cần khớp một bản ghi `eutr_references` bất
kỳ của document đó — không bắt buộc cùng một bản ghi (FR-047/FR-048); kết quả lọc MUST hỗ trợ phân
trang như danh sách gốc (FR-049); không cung cấp điều kiện nào rồi Search MUST hiển thị lại toàn bộ
danh sách.
...
**Testing bổ sung (Update 19)**: kiểm thử thủ công theo `quickstart.md` kịch bản 18 đến 18i — xác
nhận `/eutr/documents/add` không còn route khớp; popup Add có Valid from/Valid to editable, document
tạo ra dùng đúng giá trị hiển thị tại thời điểm Upload, chặn khi Valid from > Valid to; Edit mở đúng
popup Add ở chế độ sửa (Type khóa, chip chỉ đọc, không control Upload), Step hiển thị/sửa được cho
mọi Type kể cả PO; Save chỉ đổi Step (kiểm tra DB: mọi dòng `eutr_references` của document cùng đổi
`StepId`, giữ nguyên `RefValue`/`RefType`/số lượng) và/hoặc Valid from/to; document Type trống ẩn
Step, chỉ sửa Valid from/to; cột Conditions hiển thị đúng `RefValue` phẳng, dedupe khi Type="PO" khớp
nhiều Step cùng 1 mã PO; đóng popup không Save thì không có thay đổi nào được lưu.

**Testing bổ sung (Update 20)**: kiểm thử thủ công theo `quickstart.md` kịch bản 19 (19a/19b) — ở
popup Add, chọn Type khác "PO" chưa từng gán Step nào (màn Assign Steps của `006-eutr-reference-types`
rỗng cho Type đó), xác nhận combobox Step trống và Upload vẫn vô hiệu hóa; sang một Type đã gán ≥2
Step, xác nhận danh sách chỉ gồm đúng các Step đã gán và dòng đầu tiên được chọn sẵn; đổi Type khác
xác nhận danh sách/mặc định cập nhật lại; ở popup Edit của 1 document có Step đã bị gỡ khỏi Assign
Steps sau khi tạo, xác nhận Step hiện tại của document vẫn hiển thị và được chọn (không bị thay bằng
mặc định khác); Type = "PO" không đổi (Step vẫn ẩn hoàn toàn).

**Testing bổ sung (Update 21)**: kiểm thử thủ công theo `quickstart.md` kịch bản 20 (20a/20b) — mở
danh sách chính, xác nhận search box hiển thị Type/Step name/Conditions/Search; chọn 1 Type có ít
nhất 1 document liên kết, bấm Search, kiểm tra tab Network (`POST /eutr-documents/get-all` kèm filter
`TypeId`), xác nhận bảng chỉ còn document khớp; lặp lại với Step name và Conditions (từng điều kiện
riêng lẻ và kết hợp cả 3); xóa hết điều kiện rồi Search lại, xác nhận danh sách đầy đủ hiển thị lại;
chọn tổ hợp không document nào khớp, xác nhận bảng hiển thị "No data" (không lỗi).
...
**Update 18**: 1 field DTO nullable + 1 dòng ternary backend, truyền thêm 1 tham số qua 3 file
frontend hiện có — không file mới, không migration.
**Update 19**: Thay đổi phạm vi **giảm** đáng kể so với vẻ ngoài "nhiều Update" — chủ yếu là **xóa
code**: 1 trang riêng (Add) + 2 popup (Edit đơn giản, Assign condition) bị xóa hoàn toàn; backend xóa
1 service (`IEutrConditionAssignmentService`), 1 entity+repository (`EutrReferenceDetails`), 5
endpoint, ~11 DTO/projection; sửa 5 file backend hiện có (không file backend mới nào); frontend xóa 3
component + 5 use case, đổi tên 2 file (dialog hợp nhất, use case sửa Step), sửa ~8 file hiện có
(không component mới nào). Kết quả cuối: **1 màn hình** (list) + **1 popup duy nhất** dùng chung cho
cả Add và Edit — đơn giản hơn hẳn cấu trúc "1 trang + 3 popup khác nhau tùy Type" đã tồn tại qua các
Update 11-16.
**Update 20**: **0 file backend mới, 0 file backend sửa** — toàn bộ hạ tầng đọc đã có sẵn từ feature
`006-eutr-reference-types`. Frontend: sửa đúng 1 file hiện có (`EutrDocumentsFormDialog.jsx` — thêm
gọi `GetByTypeIdEutrReferenceTypeDetailsUseCase` đã có sẵn khi Type đổi/khi mở Edit, lọc + mặc định
chọn dòng đầu, đảm bảo Step hiện tại của Edit luôn hiển thị được); không thêm use case/repository/
domain entity mới ở frontend (toàn bộ `GetByTypeIdEutrReferenceTypeDetailsUseCase`/
`RestEutrReferenceTypeDetailsRepository`/`eutrReferenceTypeDetailsApi`/DI đã tồn tại sẵn từ feature
`006`, chỉ import và dùng lại) — cùng mức "0 backend mới" đã đạt được ở Update 17.
**Update 21**: Backend — sửa 2 file hiện có (`IEutrReferencesRepository.cs`/`EutrReferencesRepository.cs`
thêm 1 method `GetMatchingDocumentIdsAsync`; `EutrDocumentsService.cs` — sửa `GetPagedAsync` để tách
3 filter ảo và gọi method mới) — không file mới, không migration DB mới, không đổi
route/policy/request/response shape của endpoint `get-all`. Frontend — 1 file mới
(`EutrDocumentsFilterBar.jsx`) + sửa 2 file hiện có (`useEutrDocumentsData.js` thêm tham số
`defaultFilters`; `index.jsx` thêm state search + `handleSearch`) — không thêm dependency mới (tái
dùng `GetEutrReferenceTypesUseCase`/`GetEutrStepsUseCase` đã có sẵn từ Update 15/20).
...
**Re-check sau Update 19** (hợp nhất hoàn toàn Add/Edit vào một popup; xóa hoàn toàn trang Add cũ +
popup Assign condition; đơn giản hóa cột Conditions): vẫn PASS cả 5 nguyên tắc — mọi thay đổi backend
(đơn giản hóa `PUT {id}/step`, mở rộng SQL `GetStepInfoByDocumentIdsAsync`, xóa service/entity/
repository/endpoint không còn dùng) nằm đúng layer đã định (Application/Infrastructure, không có SQL
nào lọt lên Controller — Nguyên tắc I); component frontend hợp nhất qua prop `mode` tiếp tục **clone
đúng 1 component đã hoạt động** (`EutrDocumentsAddDialog.jsx` của Update 15) thay vì viết lại từ đầu,
và việc "sửa Step" mới tái dùng nguyên vẹn route/policy `{id}/step` đã có từ Update 12 — chỉ đổi hành
vi nội bộ (Nguyên tắc II); không tạo entity/repository/migration DB mới nào — mọi thay đổi là **mở
rộng** abstraction sẵn có (`IEutrReferencesRepository`, 2 DTO upload) hoặc **xóa bớt** code không còn
đường gọi (Nguyên tắc III, mức áp dụng cao nhất của "Reuse Existing Backend" trong toàn bộ lịch sử
feature — không thêm hạ tầng mới, chỉ đơn giản hóa hạ tầng cũ); không có UI label tiếng Việt phát sinh
(Nguyên tắc IV không đổi); route `{id}/step` giữ nguyên path/policy `EutrDocuments.Update` (chỉ đổi
hành vi + đổi tên DTO nội bộ), 5 endpoint bị xóa không để lại route "rỗng"/policy mồ côi nào (dọn dẹp
đầy đủ, không phải xóa nửa vời — Nguyên tắc V không đổi). Quyết định xóa hẳn (thay vì giữ dead code
như Update 15) được ghi nhận là một **đảo ngược có chủ đích** của quyết định trước đó (research Quyết
định 53/57) — không vi phạm nguyên tắc nào vì constitution không yêu cầu giữ nguyên mọi quyết định
trước đó bất biến, chỉ yêu cầu quyết định hiện tại không vi phạm nguyên tắc và được ghi lại rõ ràng lý
do (đã thực hiện ở research.md).

**Re-check sau Update 20** (lọc Step theo Assign Steps của Type; mặc định chọn dòng đầu): vẫn PASS cả
5 nguyên tắc — toàn bộ thay đổi nằm trong `presentation/` của 1 component frontend hiện có
(`EutrDocumentsFormDialog.jsx`), không đụng domain/infrastructure/application/backend nào (Nguyên tắc
I); tái sử dụng **nguyên vẹn** use case/repository/API client/entity đã tồn tại sẵn từ feature
`006-eutr-reference-types` (`GetByTypeIdEutrReferenceTypeDetailsUseCase`/
`repositories.eutrReferenceTypeDetails`) thay vì viết lại logic gọi API hay tạo use case trùng lặp —
đây là mức áp dụng cao của Nguyên tắc II (Reference-Pattern Reuse) và Nguyên tắc III (Reuse Existing
Backend): **0 dòng backend mới**, không entity/repository/endpoint/migration nào phát sinh, cùng tinh
thần "0 backend mới" đã đạt được ở Update 17; endpoint tiêu thụ
(`GET /api/eutr-reference-type-details/by-type/{typeId}`) dùng chung policy `EutrReferenceTypes.
ReadOne` đã có sẵn — cùng đúng tiền lệ dialog này đã gọi không điều kiện `GET /api/eutr-reference-
types`/`GET /api/eutr-steps` (2 endpoint thuộc policy của 2 feature khác) từ trước Update 20, nên việc
thêm 1 lệnh gọi cross-feature thứ ba không phải ngoại lệ mới cho feature này (Nguyên tắc V — không cần
policy/route/menu mới); không có UI label tiếng Việt phát sinh (Nguyên tắc IV không đổi).

**Re-check sau Update 21** (thêm search box lọc Type/Step name/Conditions): vẫn PASS cả 5 nguyên tắc —
method mới `GetMatchingDocumentIdsAsync` nằm đúng `Infrastructure/Repositories/EutrReferencesRepository.cs`,
chỉ được gọi qua interface `IEutrReferencesRepository` từ `Application/Services/EutrDocumentsService.cs`
(`GetPagedAsync`), không có SQL nào lọt lên `EutrDocumentsController` (Nguyên tắc I); cơ chế "tách
filter ảo rồi IN-filter" tái dùng **nguyên vẹn** `Operator = "in"` đã có sẵn của generic repository và
clone đúng tiền lệ "tính id ở nơi khác rồi IN-filter query cha" đã tồn tại tại
`AllCompliancesController`/`ViewCompliancesController` — không phát minh cơ chế lọc mới (Nguyên tắc
II); endpoint `get-all` không đổi path/policy/request shape công khai, chỉ mở rộng hành vi nội bộ
service (Nguyên tắc III — đúng tinh thần "Reuse Existing Backend", không tạo endpoint/DTO mới cho một
nhu cầu có thể giải quyết bằng cách mở rộng endpoint hiện có); frontend clone **nguyên vẹn**
`ComplianceFilterBar.jsx` + wiring `handleSearch`/`searchFilters` của `compliance-master/index.jsx`
(Nguyên tắc II), tái dùng `GetEutrReferenceTypesUseCase`/`GetEutrStepsUseCase` đã có sẵn thay vì tạo
use case/API mới (Nguyên tắc III); toàn bộ nhãn search box (Type, Step name, Conditions, Search,
"All") bằng tiếng Anh theo FR-050/FR-038 (Nguyên tắc IV không đổi); không route/DTO/policy/menu mới
(Nguyên tắc V không đổi).

**Re-check sau Update 22** (Edit cho phép thêm/xóa chip Value khi Type khác "PO"): vẫn PASS cả 5
nguyên tắc — logic đối chiếu (diff insert/delete) nằm đúng `Application/Services/
EutrDocumentsService.cs`, 2 method ghi mới nằm đúng `Infrastructure/Repositories/
EutrReferencesRepository.cs` sau `IEutrReferencesRepository`, không có SQL nào lọt lên
`EutrDocumentsController` (Nguyên tắc I); frontend tái dùng **nguyên vẹn** component
`EutrAddValueAutocomplete.jsx` đã có từ Update 15 (không viết lại logic gợi ý/giới hạn 1 chip) và
helper `toRefValue` đã có sẵn trong cùng file (Nguyên tắc II); backend mở rộng đúng 1 field DTO +
1 method service + 2 method repository trên hạ tầng `eutr_references`/`PUT {id}/step` đã tồn tại từ
Update 7/19, không tạo entity/controller/route mới cho một nhu cầu giải quyết được bằng cách mở rộng
endpoint hiện có (Nguyên tắc III); không có UI label tiếng Việt phát sinh (Nguyên tắc IV không đổi);
...
Backend — **Update 19** (hợp nhất Add/Edit; xóa hoàn toàn luồng Assign condition/trang Add cũ —
**KHÔNG migration DB mới**, chỉ mở rộng 2 SQL/DTO đã có + xóa code không còn dùng):

```text
compliance-sys-api/src/
├── ComplianceSys.Application/
│   ├── Interfaces/Repositories/IEutrReferencesRepository.cs   # (SỬA) + Task UpdateStepIdByDocumentIdAsync(long documentId, long stepId, string updatedBy, CancellationToken ct); XÓA GetDocumentsByPoCodesAsync/GetUnassignedDocumentsPagedAsync (không còn caller)
│   ├── Dtos/Response/EutrReferenceStepInfo.cs                 # (SỬA) + string? RefValue (dùng dựng Conditions phẳng, research Quyết định 54)
│   ├── Dtos/Response/EutrDocumentsResponseDto.cs              # (SỬA) Conditions: List<ConditionGroupDto> → List<string> (research Quyết định 54)
│   ├── Dtos/Request/EutrMultiUploadFileRequest.cs             # (SỬA) + DateTime? ValidFrom, DateTime? ValidTo (research Quyết định 55)
│   ├── Dtos/Request/EutrTypeMultiUploadFileRequest.cs         # (SỬA) + DateTime? ValidFrom, DateTime? ValidTo (research Quyết định 55)
│   ├── Dtos/Request/EutrUpdateReferenceStepRequestDto.cs      # ĐỔI TÊN từ EutrUpdatePoStepRequestDto.cs (cùng shape { long StepId }, research Quyết định 56)
│   ├── Interfaces/Services/IEutrDocumentsService.cs           # (SỬA) + Task UpdateReferenceStepAsync(long documentId, long stepId, string userEmail, CancellationToken ct); XÓA GetUnassignedPagedAsync/GetPoReferencesAsync
│   ├── Services/EutrDocumentsService.cs                       # (SỬA) + implement UpdateReferenceStepAsync (gọi _referencesRepository.UpdateStepIdByDocumentIdAsync); AttachStepAndConditionInfoAsync đổi cách dựng Conditions (Distinct RefValue, bỏ gọi IEutrReferenceDetailsRepository); XÓA GetUnassignedPagedAsync/GetPoReferencesAsync
│   ├── Services/EutrUploadService.cs                          # (SỬA) 2 nơi gán cứng ValidFrom=Today/ValidTo=MaxValidTo đổi thành request.ValidFrom ?? Today / request.ValidTo ?? MaxValidTo (UploadMultipleToSharePointAndSaveDataAsync + UploadMultipleForReferenceTypeAsync); XÓA UploadManualMultipleToSharePointAndSaveDataAsync
│   └── XÓA: Interfaces/Services/IEutrConditionAssignmentService.cs, Services/EutrConditionAssignmentService.cs, Dtos/Request/EutrManualMultiUploadFileRequest.cs, Dtos/Request/EutrAssignConditionsRequestDto.cs (+Validator), Dtos/Request/EutrConditionRowDto.cs, Dtos/Request/EutrUpdateConditionAssignmentRequestDto.cs (+Validator), Dtos/Response/EutrDocumentConditionAssignmentDto.cs, Dtos/Response/ConditionGroupDto.cs, Dtos/Response/EutrConditionGroupRow.cs, Dtos/Request/EutrDocumentsListPoReferencesRequestDto.cs, Dtos/Response/EutrDocumentsPoReferenceDto.cs, Dtos/Response/EutrDocumentsPoReferenceItemDto.cs, Dtos/Response/EutrReferencePoDocumentInfo.cs
├── ComplianceSys.Domain/Entities/
│   └── XÓA: EutrReferenceDetails.cs
├── ComplianceSys.Infrastructure/
│   ├── Repositories/EutrReferencesRepository.cs               # (SỬA) + implement UpdateStepIdByDocumentIdAsync (UPDATE StepId theo DocumentId); GetStepInfoByDocumentIdsAsync: + r.RefValue vào SELECT; XÓA GetDocumentsByPoCodesAsync/GetUnassignedDocumentsPagedAsync; GIỮ NGUYÊN SQL 2 bước của DeleteByDocumentIdAsync (research Quyết định 58)
│   ├── DependencyInjection.cs                                  # (SỬA) - services.AddScoped<IEutrConditionAssignmentService, EutrConditionAssignmentService>(); - services.AddScoped<IEutrReferenceDetailsRepository, EutrReferenceDetailsRepository>();
│   └── XÓA: Repositories/EutrReferenceDetailsRepository.cs
└── ComplianceSys.Api/Controllers/
    ├── EutrDocumentsController.cs                              # (SỬA) action {id}/step gọi _eutrDocumentsService.UpdateReferenceStepAsync thay vì _conditionAssignmentService.UpdatePoStepAsync; XÓA constructor param IEutrConditionAssignmentService; XÓA action GetUnassigned/AssignConditions/GetConditionAssignment/UpdateConditionAssignment/GetPoReferences
    └── SharepointController.cs                                 # (SỬA) XÓA action eutr-upload-manual-multi + constructor param không còn cần (nếu chỉ dùng riêng cho action đó)
```

Frontend — **Update 19** (hợp nhất Add/Edit vào 1 popup; xóa hoàn toàn trang Add cũ + popup Assign
condition):

```text
compliance-client/src/
├── application/usecases/
│   ├── eutr-documents/UpdateEutrDocumentPoStepUseCase.js       # ĐỔI TÊN → UpdateEutrDocumentReferenceStepUseCase.js (cùng shape execute(id, stepId), research Quyết định 57)
│   └── XÓA: eutr-documents/AssignEutrConditionsUseCase.js, eutr-documents/GetEutrDocumentConditionAssignmentUseCase.js, eutr-documents/UpdateEutrConditionAssignmentUseCase.js, eutr-documents/GetEutrDocumentsUnassignedUseCase.js, eutr-documents/GetEutrDocumentsPoReferencesUseCase.js
├── domain/interfaces/IEutrDocumentsRepository.js                # (SỬA) XÓA getUnassigned/assignConditions/getConditionAssignment/updateConditionAssignment/getPoReferences
├── infrastructure/
│   ├── api/eutrDocumentsApi.js                                  # (SỬA) XÓA getUnassigned/assignConditions/getConditionAssignment/updateConditionAssignment/listPoReferences
│   ├── repositories/RestEutrDocumentsRepository.js              # (SỬA) XÓA implementation tương ứng
│   ├── repositories/RestSharePointRepository.js                 # (SỬA) uploadEutrFilesMulti(files, poCode, typeId, validFrom, validTo)/uploadEutrFilesMultiByType(..., validFrom, validTo) — thêm 2 field vào FormData; XÓA uploadEutrManualFilesMulti
│   └── domain/interfaces/ISharePointRepository.js               # (SỬA) XÓA uploadEutrManualFilesMulti(_files)
├── application/usecases/sharepoint/UploadToSharePointUseCase.js  # (SỬA) executeEutrMulti(files, poCode, typeId, validFrom, validTo)/executeEutrMultiByType(..., validFrom, validTo); XÓA executeManualMulti
└── presentation/
    ├── pages/eutr-documents/
    │   ├── index.jsx                                            # (SỬA) onEdit KHÔNG còn rẽ nhánh theo refType — luôn mở EutrDocumentsFormDialog mode="edit" initialData={row}; XÓA state modalOpen/modalData, assignDialogOpen/assignDialogData; XÓA import EutrDocumentsModal/AssignConditionDialog
    │   ├── EutrDocumentsAdd.jsx                                  # XÓA HOÀN TOÀN (cùng bảng List PO/Screen1/Screen2/bảng "chưa gán")
    │   ├── hooks/useEutrDocumentsColumns.jsx                     # (SỬA) cột "conditions": renderCell dùng <MultiValueChips values={row.conditions} /> (bỏ nhóm theo CONDITION_TYPE_OPTIONS)
    │   └── components/
    │       ├── EutrDocumentsAddDialog.jsx                        # ĐỔI TÊN → EutrDocumentsFormDialog.jsx; + prop mode/'add'|'edit', initialData; + 2 DatePicker Valid from/Valid to (mode add, validate validFrom≤validTo); mode edit: Type disabled, chip read-only (ẩn EutrAddValueAutocomplete), Step hiện cho mọi Type (research Quyết định 60), nút Save thay Upload, onSave gọi tuần tự updateEutrDocumentsUseCase rồi (nếu Step hiện) updateEutrDocumentReferenceStepUseCase
    │       └── XÓA: EutrDocumentsModal.jsx, AssignConditionDialog.jsx
    ├── menu-items/ComplianceSystem.jsx                           # (SỬA, nếu có entry riêng cho route Add) XÓA entry trỏ /eutr/documents/add
    └── (không đổi) hooks/useEutrDocumentsData.js, components/EutrDocumentsActionCell.jsx, components/EutrFileViewerDialog.jsx, components/MultiValueChips.jsx
├── (SỬA) app/routes/groups/MainRoutes.jsx                        # XÓA route "/eutr/documents/add" → EutrDocumentsAdd
└── (SỬA, nếu cần) utils/helpers.js                               # XÓA CONDITION_TYPE_OPTIONS nếu không còn dùng ở nơi khác trong repo (xác nhận lại lúc implement)
```

Frontend — **Update 20** (lọc Step theo Assign Steps của Type, mặc định chọn dòng đầu — sửa đúng 1
file, KHÔNG thêm use case/repository/domain nào mới; toàn bộ hạ tầng tiêu thụ đã tồn tại sẵn từ
feature `006-eutr-reference-types`):

```text
compliance-client/src/presentation/pages/eutr-documents/components/
└── EutrDocumentsFormDialog.jsx   # (SỬA — Update 20) + import GetByTypeIdEutrReferenceTypeDetailsUseCase (đã có sẵn từ feature 006, application/usecases/eutr-reference-type-details/); instantiate với repositories.eutrReferenceTypeDetails (đã đăng ký sẵn trong di/repositories.js) — không tạo file domain/infrastructure/application mới nào; gọi execute(type.id) mỗi khi `type` đổi (mode add) hoặc ngay khi mở popup ở mode edit (dùng Type hiện tại từ initialData, đã khóa); map kết quả (stepId/stepName) sang { id, name } (ưu tiên đối chiếu với `steps` đầy đủ đã tải bởi getEutrStepsUseCase để giữ đúng object identity cho isOptionEqualToValue, fallback dùng thẳng stepName nếu không khớp); setSteps(filteredSteps); mode add: setStep(filteredSteps[0] ?? null) ngay sau khi tải xong (FR-044); mode edit: nếu Step hiện tại (từ initialData) không có trong filteredSteps thì chèn thêm chính Step đó vào đầu mảng hiển thị, KHÔNG đổi giá trị `step` đang chọn (FR-045); Type = "PO" không đổi (Step tiếp tục ẩn qua isPoType, FR-010/Quyết định 51/60 không đổi)
```

Backend — **Update 21** (search box lọc Type/Step name/Conditions — sửa 2 file hiện có, KHÔNG file
mới, KHÔNG migration DB mới, KHÔNG đổi route/policy/request-response shape công khai):

```text
compliance-sys-api/src/
├── ComplianceSys.Application/Interfaces/Repositories/
│   └── IEutrReferencesRepository.cs   # (SỬA) + Task<List<long>> GetMatchingDocumentIdsAsync(long? typeId, long? stepId, string? conditionsQuery, CancellationToken ct = default)
├── ComplianceSys.Infrastructure/Repositories/
│   └── EutrReferencesRepository.cs    # (SỬA) + implement GetMatchingDocumentIdsAsync: SELECT d.Id FROM eutr_documents d WHERE (3 EXISTS độc lập trên eutr_references theo RefType/StepId/RefValue LIKE, mỗi điều kiện chỉ áp dụng khi tham số tương ứng khác null) — xem data-model.md
└── ComplianceSys.Application/Services/
    └── EutrDocumentsService.cs        # (SỬA) GetPagedAsync: tách filter Column ∈ {"TypeId","StepId","Conditions"} ra khỏi request.Filters trước khi gọi base; nếu có ≥1, gọi _referencesRepository.GetMatchingDocumentIdsAsync(...) — rỗng thì trả PagedResult rỗng ngay, ngược lại thêm FilterRequest{Column="Id",Operator="in",Value=joinedIds} vào filters còn lại rồi gọi base.GetPagedAsync như cũ
```

Frontend — **Update 21** (search box — 1 component mới nhỏ, sửa 2 file hiện có, tái dùng use case
Type/Step đã có sẵn):

```text
compliance-client/src/presentation/pages/eutr-documents/
├── components/
│   └── EutrDocumentsFilterBar.jsx   # MỚI: clone cấu trúc ComplianceFilterBar.jsx (Box hàng ngang) — Autocomplete Type (getEutrReferenceTypesUseCase, đã dùng ở popup Add), Autocomplete Step name (getEutrStepsUseCase, TOÀN BỘ eutr_steps không lọc theo Type — khác Assign Steps của Update 20), TextField Conditions, Button Search (startIcon SearchIcon, mẫu ComplianceFilterBar) — không có Clear button (spec chỉ yêu cầu đúng 4 control)
├── hooks/useEutrDocumentsData.js    # (SỬA) đổi chữ ký useEutrDocumentsData(defaultFilters = []) (mẫu useComplianceMasterData) — gộp defaultFilters vào filterPayload trước khi gọi getPagingEutrDocumentsUseCase.execute(...)
└── index.jsx                        # (SỬA) + state typeFilter/stepFilter/conditionsFilter/searchFilters; handleSearch(): build filters [{column:"TypeId",operator:"=",value},{column:"StepId",operator:"=",value},{column:"Conditions",operator:"like",value}] (chỉ thêm điều kiện đang có giá trị), setSearchFilters(filters), reset paginationModel.page về 0 (mẫu compliance-master/index.jsx handleSearch); render <EutrDocumentsFilterBar ... onSearch={handleSearch} /> phía trên DataGrid
```

Backend — **Update 22** (Edit cho phép thêm/xóa chip Value khi Type khác "PO" — sửa 1 DTO + 3 file
hiện có, KHÔNG file mới, KHÔNG migration DB mới, KHÔNG đổi route/policy):

```text
compliance-sys-api/src/
├── ComplianceSys.Application/Dtos/Request/
│   └── EutrUpdateReferenceStepRequestDto.cs   # (SỬA) + List<string>? RefValues { get; set; } (null = giữ hành vi cũ chỉ update StepId; có giá trị = kích hoạt đối chiếu thêm/xóa)
├── ComplianceSys.Application/Interfaces/Repositories/
│   └── IEutrReferencesRepository.cs           # (SỬA) + Task DeleteByDocumentIdAndRefValuesAsync(long documentId, IEnumerable<string> refValues, CancellationToken ct = default); + Task AddReferencesAsync(long documentId, long stepId, byte refType, IEnumerable<string> refValues, string createdBy, CancellationToken ct = default)
├── ComplianceSys.Infrastructure/Repositories/
│   └── EutrReferencesRepository.cs            # (SỬA) implement 2 method mới trên bằng raw SQL (xem data-model.md) — DeleteByDocumentIdAndRefValuesAsync dọn kèm eutr_reference_details mồ côi theo cùng mẫu 2 bước của DeleteByDocumentIdAsync (Update 11)
└── ComplianceSys.Application/Services/
    └── EutrDocumentsService.cs                # (SỬA) UpdateReferenceStepAsync(documentId, stepId, refValues, userEmail, ct): refValues == null → giữ nguyên hành vi cũ (không transaction); khác null → mở transaction (_unitOfWork, mẫu DeleteAsync/DeleteMultiAsync Update 9), đọc RefValue/RefType hiện có qua GetStepInfoByDocumentIdsAsync (đã có sẵn, KHÔNG method đọc mới), tính Except 2 chiều, gọi DeleteByDocumentIdAndRefValuesAsync/AddReferencesAsync rồi UpdateStepIdByDocumentIdAsync, Commit/Rollback
```

Backend — **Controller** (sửa 1 dòng gọi service, DTO field mới tự bind qua model binding, không đổi
route/policy):

```text
compliance-sys-api/src/ComplianceSys.Api/Controllers/
└── EutrDocumentsController.cs   # (SỬA) UpdateReferenceStep(...): truyền thêm dto.RefValues vào _eutrDocumentsService.UpdateReferenceStepAsync(id, dto.StepId, dto.RefValues, userEmail, ct)
```

Frontend — **Update 22** (sửa 5 file hiện có, KHÔNG component mới — tái dùng nguyên vẹn
`EutrAddValueAutocomplete.jsx` đã có từ Update 15):

```text
compliance-client/src/
├── presentation/pages/eutr-documents/components/
│   └── EutrDocumentsFormDialog.jsx   # (SỬA) showEditableChips = isEdit && initialData?.refType != null && !isPoType (cùng cách tính showStep); render EutrAddValueAutocomplete + Chip onDelete khi (!isEdit || showEditableChips) thay vì (!isEdit) đơn thuần; canSubmit (mode edit) + điều kiện (!showEditableChips || chips.length > 0); handleSave: refValues = showEditableChips ? chips.map(toRefValue) : undefined, truyền vào updateEutrDocumentReferenceStepUseCase.execute(id, step.id, refValues)
├── infrastructure/api/eutrDocumentsApi.js               # (SỬA) updateReferenceStep(id, stepId, refValues) => axiosInstance.put(`/eutr-documents/${id}/step`, { stepId, refValues })
├── infrastructure/repositories/RestEutrDocumentsRepository.js   # (SỬA) updateReferenceStep(id, stepId, refValues) truyền refValues qua API client
├── domain/interfaces/IEutrDocumentsRepository.js        # (SỬA) chữ ký updateReferenceStep(_id, _stepId, _refValues)
...
> **Cập nhật (Update 19)**: Vế "eutr-templates (routing trang riêng cho Add)" ở trên mô tả kiến trúc
> **ban đầu** (route `/eutr/documents/add`), đã bị thay thế hoàn toàn từ Update 15/16 (popup) và route
> đó đã bị **xóa hẳn** ở Update 19 (không còn tồn tại trong `MainRoutes.jsx`). Mẫu tham chiếu hiện
> hành cho cả Add và Edit là **eutr-masters** (popup dùng chung, qua prop `mode`) — không còn mẫu
> "trang riêng" nào áp dụng cho feature này nữa.
...
> mô tả kiến trúc hiện tại (xem dòng Update 19 ngay dưới để biết trạng thái hiện hành).
