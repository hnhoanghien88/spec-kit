...
- **FR-012**: For each distinct combination whose lookup (FR-011) returns data, before inserting that data System MUST delete only the existing local Variant Attribute record(s) that share that exact Product Code + Config ID — leaving records for every other combination in the store untouched. System MUST NOT clear or truncate the entire local Variant Attribute store as part of a run.
...
- **FR-014**: When a lookup (FR-011) for a given combination returns no data, System MUST NOT create, delete, or otherwise modify any record for that combination — any existing local record for that Product Code + Config ID from an earlier run remains as-is — and MUST continue processing the remaining combinations without treating this as an error.
...
- **Variant Attribute Sync Record (local, persisted)**: The stored result of one distinct Product Code + Config ID combination's Variant Attribute lookup. For each distinct combination processed in a run whose lookup returned data, the run first deletes any existing record(s) for that exact Product Code + Config ID (FR-012), then inserts the newly retrieved data (FR-013). Records for combinations not processed in the current run — because they weren't among the run's distinct Sales Line combinations, or their lookup returned no data (FR-014) — are left untouched and may carry over from earlier runs.
...
- **SC-004**: After a run, the local Sales Line Sync store contains only records produced by that run — 0 records carried over from any earlier run. The local Variant Attribute Sync store contains that run's fresh data for every distinct Product + Config combination processed in the run — 0 stale rows for those combinations — while records for combinations not processed in the current run may still carry over from earlier runs.
...
- The local Sales Line store follows the existing "clear entire store, then fully repopulate" pattern already used by comparable local sync/snapshot tables in this system, rather than an "insert if not already present" pattern — this was confirmed via `/speckit-specify` clarification (see below); every run's Sales Line data reflects only that run's results.
- The local Variant Attribute store instead follows a per-combination "delete by Product Code + Config ID, then insert" pattern — updated via the 2026-08-20 `/speckit-specify` update below. The entire store is never truncated in one step; only the rows for combinations actually being (re-)inserted in the current run are replaced, and rows for combinations not touched by the current run persist from earlier runs.
...
### `/speckit-specify` Update — Variant Attribute Store Deletion Scope (2026-08-20)

**Change requested**: The Variant Attribute phase must no longer clear the entire `compl_sync_variant_attributes` table at the start of a run. Instead, deletion must be scoped per Product Code + Config ID combination — only the row(s) for the combination about to be (re-)inserted are deleted first, immediately before that combination's newly retrieved data is inserted. Combinations not part of the current run (or whose lookup returned no data) are left untouched, so their previously synced rows persist.

**Rationale**: A full truncate of `compl_sync_variant_attributes` on every run discards data for Product/Config combinations that simply weren't part of the current run's Sales Line data (e.g., a transient ERP gap or a narrower ad-hoc run), even though nothing about those combinations actually changed. Scoping the delete to only the combinations being refreshed avoids this unnecessary data loss.

**Applied to**: FR-012, FR-014, the Variant Attribute Sync Record (Key Entities), SC-004, and the Assumptions section. This changes the Variant Attribute store's behavior only — the Sales Line store (FR-008, `compl_sync_sales_line`) is unaffected and continues to be fully cleared and rebuilt each run.
