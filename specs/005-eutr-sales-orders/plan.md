...
**Update 22 (2026-09-07)**: Changes Download (View's Download, Update 10; Overview's per-row Download,
Update 13) from always packaging both the per-template folders (Update 10/13) and the All folder
(Update 21) into one zip, to first showing a choice popup — spec FR-152..FR-160. A new small shared
dialog, `DownloadFormatDialog.jsx` (cloned from the existing `ConfirmDialog.jsx`'s Dialog/Actions shape,
Constitution Principle II — no ready-made two-option choice dialog exists anywhere in this codebase to
reuse as-is, confirmed by a full search of `004-eutr-documents`/`003-eutr-templates`/`presentation/
components/`), presents exactly 2 mutually-exclusive options — **Combined (All)** and **By Template** —
with no option pre-selected and its confirm button disabled until one is chosen. This is a **100%
frontend-only, zero-backend-change** update: `POST /api/eutr-documents/download-zip` already treats
`folders` as an opaque, client-supplied list (Update 10/21, Decisions 37/69) with no server-side notion
of "All" vs "template," so the only change needed is which of the two already-correctly-computed folder
sets (`templateFolders` from Update 10/13, All-folder entries from Update 21) each screen concatenates
into that list — gated by the dialog's chosen format instead of always concatenating both.
`ViewSalesOrderPage.jsx`'s `buildDownloadFolders` becomes format-aware with no new fetch (its inputs are
already loaded on mount regardless of Download, since the always-visible All chip/Template Checklist
depend on them too). `SalesOrderOverviewPage.jsx` gains one new `downloadDialogRow` state (a single
value, not a per-row `Set`, since only one modal can be meaningfully open at a time) and a genuine
optimization Update 21 did not have: when the user picks **By Template**, the on-demand default-template
2-call fetch (Update 21) is skipped entirely, since only the Combined format's output needs it. See
research.md Decisions 71-73.
...
- **I. Layered Clean Architecture** — PASS. Backend change stays in `ComplianceSys.Application`
  ...
  **Update 22**: no backend layer is touched at all (`download-zip` reused byte-for-byte). Frontend: the
  new `DownloadFormatDialog.jsx` is a presentation-layer component consumed directly by two other
  presentation-layer files (`ViewSalesOrderPage.jsx`/`SalesOrderOverviewPage.jsx`) within the same
  feature folder — not a cross-feature import, no new layer introduced; it renders no use case/repository
  call of its own (`onConfirm` only reports the chosen `format` string back to its caller, which already
  owns the existing, already-layered download chains). No new interface, repository, or use-case file, no
  layer skipped or bypassed.
...
- **Update 21**: the concrete reference for "flatten a tree into per-node entries" is this same file's
  ...
  **Update 22**: the concrete reference for "a lightweight modal with Cancel + one confirm action" is
  this codebase's own `presentation/components/ConfirmDialog.jsx`, already reused by
  `CloneTemplateDialog.jsx` (`003-eutr-templates`) — `DownloadFormatDialog.jsx` clones its
  `Dialog`/`DialogTitle`/`DialogContent`/`DialogActions` structure and `open`/`onClose`/`onConfirm` prop
  shape, extending it with a `RadioGroup` rather than inventing an unrelated dialog pattern (a full search
  of `004-eutr-documents`/`003-eutr-templates`/`presentation/components/` found no existing two-option
  choice modal to reuse as-is — see research.md Decision 71). `templateFolders`/the All-folder entries
  themselves are reused exactly as Update 10/13/21 already compute them — this update reuses those
  computations rather than re-deriving folder content a third way.
- **III. Reuse Existing Backend** — PASS, with a verified, scoped gap. `POST /api/dynamics/reference`
  ...
  **Update 22**: PASS, zero backend change of any kind — `POST /api/eutr-documents/download-zip` is
  reused exactly as Update 21 left it (Decision 69), with no new field, endpoint, or server-side
  awareness of which format the client chose (Decision 71/72) — the existing backend already fully
  satisfies this update's needs.
...
- **Update 22**: all new `DownloadFormatDialog.jsx` code and the edits to `ViewSalesOrderPage.jsx`/
  `SalesOrderOverviewPage.jsx` (dialog wiring, format-aware `buildDownloadFolders`/`handleDownload`) MUST
  carry Vietnamese comments matching this feature's existing style. This update introduces exactly 2 new
  English UI labels — **"Combined (All)"** and **"By Template"** (the dialog's radio option text) — and a
  dialog title/confirm-button label (e.g. "Download", already an existing English label on this screen's
  own Download button/icon tooltip). This is consistent with, not a new deviation from, this feature's
  already-established pattern of English control/action labels (Download, Edit / Map File, Back, View —
  Update 4/9/10) alongside otherwise-Vietnamese surrounding text; the spec's own Update 22 clarification
  explicitly delegates the exact wording to implementation ("đặt tên lại cho hợp lý"), resolved here as
  these two labels per research.md Decision 71.
- **V. Routing & Menu Registration** — PASS, already satisfied. Route (`/eutr/sales-orders` →
  ...
  **Update 22**: no route/menu/policy change at all — the new `DownloadFormatDialog` is a modal rendered
  inline within the already-reachable View/Overview screens, not a new route; `download-zip` keeps its
  existing `EutrDocuments.ReadAll` policy unchanged (Update 10/21), and the default-template fetch (now
  conditionally skipped for the By Template format on Overview) keeps the same `EutrTemplates.ReadAll`
  policy Update 19 already relies on when it does run — no new action, no new policy code, no new
  ops-seeding step.
...
# Update 22 (2026-09-07) — Download shows a choice popup (Combined All / By Template) instead of
# always packaging both; zero backend change at all (download-zip reused exactly as Update 21 left
# it); frontend adds 1 new small component (DownloadFormatDialog.jsx) and edits confined to
# ViewSalesOrderPage.jsx + SalesOrderOverviewPage.jsx:
```
compliance-client/
└── src/
    └── presentation/
        ├── components/
        │   └── ConfirmDialog.jsx           # UNCHANGED (Update 22): reference shape only, not edited
        └── pages/eutr-sales-orders/
            ├── components/
            │   └── DownloadFormatDialog.jsx  # NEW (Update 22): Dialog/DialogTitle/DialogContent/
            │                                  #       DialogActions cloned from ConfirmDialog.jsx's
            │                                  #       shape + a RadioGroup with 2 options
            │                                  #       ('combined' -> "Combined (All)", 'byTemplate'
            │                                  #       -> "By Template"); local `format` state
            │                                  #       (useState(null), no pre-selection); confirm
            │                                  #       button disabled until `format` is set; props:
            │                                  #       { open, onClose, onConfirm(format) }
            │                                  #       (research.md Decision 71)
            ├── ViewSalesOrderPage.jsx      # EDIT (Update 22): Download button's onClick now opens
            │                                 #       DownloadFormatDialog instead of calling
            │                                 #       handleDownload directly (new
            │                                 #       downloadDialogOpen state); buildDownloadFolders
            │                                 #       takes a `format` argument, returning only
            │                                 #       templateFolders (byTemplate) or only the
            │                                 #       All-folder entries (combined, with the existing
            │                                 #       FR-147 empty fallback) — never both; handleDownload
            │                                 #       takes the chosen format and passes it through; no
            │                                 #       new fetch (research.md Decision 72)
            └── SalesOrderOverviewPage.jsx  # EDIT (Update 22): new `downloadDialogRow` state
                                              #       ({ salesId, customerCode, customerName } | null,
                                              #       a single value, not per-row); Download IconButton's
                                              #       onClick sets it instead of calling handleDownload
                                              #       directly; handleDownload takes a 4th `format`
                                              #       argument gating (1) the same folder-set choice as
                                              #       View and (2) whether the on-demand
                                              #       default-template 2-call fetch (Update 21) runs at
                                              #       all — skipped entirely when format === 'byTemplate'
                                              #       (research.md Decision 73)
```

Unchanged (verified reusable as-is, no edits needed) for Update 22:
- `EutrDocumentsController.DownloadZip`, `EutrDownloadZipRequestDto.cs`, `EutrDownloadZipFolderDto.cs`,
  `EutrDownloadZipFileDto.cs` — the endpoint and every request DTO are reused byte-for-byte; the server
  has and needs zero awareness of "All" vs "template" (Decision 69/71).
- `DownloadEutrSalesOrderZipUseCase.js`, `eutrDocumentsApi.js`'s `downloadZip`,
  `IEutrDocumentsRepository.js`, `RestEutrDocumentsRepository.js` — all forward the `folders` payload
  opaquely; none reference "format"/"All"/"template" by name, so none need editing.
- `utils/treeUtils.js` (`flatToTree`, `filterFlatListByStepIds`, `flattenTreeToFolderEntries`, Update
  19/21) — reused exactly as-is; this update only changes which already-built entries get concatenated
  into the final `folders` array, not how any of them are computed.
- `downloadingSalesIds` (`SalesOrderOverviewPage.jsx`'s per-row in-flight `Set`, Update 13) — unaffected;
  the new `downloadDialogRow` state is additive and orthogonal to it.
- `MapFilePage.jsx` — has no Download button; untouched.
