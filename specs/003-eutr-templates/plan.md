...
**Frontend-only, no backend/DB/contract changes.** Both reversed decisions live entirely inside
`BulkAddStepsDialog.jsx`/`TemplateBuilderPage.jsx`, the same two files Update 12 already introduced
for the bulk-select dialog — this is an edit to existing logic in place, not a new mechanism.

- **FR-076 (reverses Update 12's FR-029 scope)** — `TemplateBuilderPage.jsx` MODIFY: the prop
  passed to `<BulkAddStepsDialog>` for "steps already present" changes from
  `existingChildStepIds={stepItems.filter(s => s.parentId === (addModal.type === 'root' ? 0 :
  selectedId)).map(s => s.stepId)}` (direct children of the target node only) to
  `usedStepIds={stepItems.map(s => s.stepId).filter(Boolean)}` (every non-null `stepId` anywhere in
  the current tree, prop renamed to reflect the new whole-tree scope). `BulkAddStepsDialog.jsx`
  MODIFY: rename the `existingChildStepIds` prop to `usedStepIds` (default `[]`); the `available =
  steps.filter(s => !usedStepIds.includes(s.id))` line itself is unchanged — the wider exclusion
  comes entirely from the caller now passing a wider array. Edit step (FR-008b, `editStep` in
  `useStepTree.js`, unrelated code path) is untouched — it still allows retargeting a node to a
  `StepId` used elsewhere in the tree.
- **FR-077 (new)** — `BulkAddStepsDialog.jsx` MODIFY: add prop `existingStepNames = []` (passed by
  `TemplateBuilderPage.jsx` as `stepItems.map(s => s.stepName).filter(Boolean)` — covers step names
  still pending auto-create, not yet in `eutr_steps`). Add a derived `isDuplicateName` (typed
  `newStepDraft.name`, trimmed/lower-cased, compared against `steps.map(s => s.name)` and
  `existingStepNames`, same normalization). `hasNewStep` becomes `Boolean(newStepDraft.name.trim())
  && !isDuplicateName` (a duplicate no longer counts toward the footer's "M selected" or gets
  included in `handleAdd`'s `fromNewStep`). The "New step name" `TextField` gains `error=
  {isDuplicateName}` + an inline `helperText` explaining the conflict. FR-007a's general
  merge-by-name mechanism elsewhere (Edit step's free-solo combobox, `StepFormRow.jsx`) is
  unaffected — this check only lives inside `BulkAddStepsDialog.jsx`'s own local state.
- No change to `useStepTree.js`, `StepFormRow.jsx`, `StepTree.jsx`, any backend file, or
  `contracts/api-endpoints.md` — verified the Update-template endpoint's `details[]` payload
  shape/validation needs no change, since this is a client-side guard that prevents certain payloads
  from ever being constructed, not a new server-side rule.
...
**Post-design re-check (2026-08-11 update 21)**: All principles still PASS. Both changes are edits to
existing presentation-layer logic inside `BulkAddStepsDialog.jsx`/`TemplateBuilderPage.jsx` — the
exact same two files Update 12 introduced for this dialog (Principle I — no new layer touched, no
service/controller/DTO involved; Principle II — reference-pattern reuse in the strongest sense,
widening/extending this feature's own existing filter expression and draft-state object rather than
introducing a new mechanism). No backend change at all (Principle III — the Update-template
endpoint's `details[]` contract is unaffected; verified against `contracts/api-endpoints.md`). No new
dependency (reuses the same `@mui/material` `TextField`/`Autocomplete` already in this file). No new
route, no new menu entry (Principle V unaffected — this is an interaction change on the
