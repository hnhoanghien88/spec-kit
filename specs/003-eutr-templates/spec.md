...
### Session 2026-07-21 (Update 16) — Status Draft/Approved thay cho versioning theo 24 giờ

- Input: "cập nhật 003-eutr-templates thêm 1 cột Status là enum sẽ định nghĩa trong helpers.js gồm
  2 trạng thái Draft, Approved, cập nhật lại logic, khi vừa tạo template, mặc định status = Draft.
  Khi user nhấn nút Approve, có hiện box xác nhận Yes/No, Yes dữ liệu chuyển sang Approved, No đóng
  box. User muốn chỉnh sửa sẽ bấm nút Request change cũng hiển thị box xác nhận Yes/No. Đồng thời
  logic lên version sẽ bỏ sau 24h, chỉ lên version khi user chuyển từ trạng thái Approve sang
  Draft."
- Q: Nút Approve và Request change nên đặt ở đâu — icon trên từng dòng TemplateListPage (giống
  Clone/Apply to Customer), nút trên TemplateBuilderPage (màn Edit), hay cả hai? → A: Đặt trên
  toolbar của TemplateListPage, cạnh nút Create Template — không phải icon trên từng dòng, không
  phải nút trên TemplateBuilderPage.
- Q: Khi Status = Approved, màn hình TemplateBuilderPage (Edit) nên xử lý thế nào? → A: Read-only
  hoàn toàn — ẩn/khóa toàn bộ nút Save, Add step, Edit step, Delete step, Root Group, Child Step và
  các trường header, kèm banner cảnh báo (giống hành vi `isReadOnly` của mockup tham chiếu
  `TemplateBuilderPage.jsx` khi status khác Draft) — cho đến khi Request change đưa template về
  Draft.
- Q: Khi xác nhận Yes ở dialog Request change (Approved → Draft), version mới được tạo lúc nào? →
  A: Ngay khi xác nhận Yes — hệ thống tạo ngay dòng mới (VersionId+1, Status=Draft, sao chép toàn
  bộ step tree + vendor mapping từ dòng Approved cũ), ẩn dòng cũ (IsHide=1). Không chờ đến lần Save
  kế tiếp trong TemplateBuilderPage.
- Change: Thêm cột **Status** (enum 2 giá trị **Draft**, **Approved**, định nghĩa tại
  `compliance-client/src/utils/helpers.js` — theo đúng quy ước các enum dùng chung khác của feature
  này như REQUIREMENT_TYPES, TAKE_FROM_OPTIONS, groupEmailType) vào bảng `eutr_templates` và hiển
  thị dạng Chip trên TemplateListPage. Xem FR-055 đến FR-063.
- Change: **Bỏ hoàn toàn** nhánh versioning dựa trên tuổi bản ghi (24 giờ) của FR-012/Update 4 —
  Save ở TemplateBuilderPage (chỉ khả dụng khi Status=Draft) từ nay LUÔN cập nhật đè lên dòng hiện
  tại, không phân biệt CreatedDate đã qua bao lâu. VersionId chỉ tăng đúng 1 thời điểm duy nhất:
  khi người dùng xác nhận **Request change** để chuyển một template đang Approved về Draft.
- Change: Template mới tạo (dialog Create Template, cả template tạo qua Clone) MẶC ĐỊNH
  Status=Draft.
- Change: Nút **Approve** (toolbar TemplateListPage, cạnh Create Template) chỉ khả dụng khi đã
  chọn (checkbox) đúng 1 dòng đang Status=Draft; nhấn Approve MUST hiển thị `ConfirmDialog` Yes/No;
  Yes → cập nhật Status=Approved ngay trên dòng hiện tại (không tạo dòng mới, không đổi VersionId);
  No → đóng dialog, không thay đổi gì.
- Change: Nút **Request change** (toolbar TemplateListPage, cạnh Create Template) chỉ khả dụng khi
  đã chọn đúng 1 dòng đang Status=Approved; nhấn Request change MUST hiển thị `ConfirmDialog`
  Yes/No; Yes → hệ thống tạo ngay một dòng mới trong eutr_templates (VersionId = VersionId cũ + 1,
  Status=Draft, IsHide=0), sao chép toàn bộ `eutr_template_details` và `eutr_template_references`
  từ dòng cũ sang TemplateId mới, và đánh dấu dòng Approved cũ IsHide=1 (giữ nguyên, không xóa —
  đóng vai trò bản ghi lịch sử bất biến của phiên bản đã Approved); No → đóng dialog, template vẫn
  giữ nguyên Status=Approved, không có thay đổi nào.

### Session 2026-07-22 (Update 17) — Kéo thả (drag-and-drop) sắp xếp step bên cạnh Move Up/Down

- Input: "cập nhật 003-eutr-templates màn hình edit. hiện tại muốn di chuyển step lên xuống phải
  bấm move up, move down, giờ thêm tính năng cho user kéo step lên xuống."
- Q: Khi kéo thả một step trong cây bước ở màn hình Edit (TemplateBuilderPage), phạm vi di chuyển
  nên giới hạn thế nào? → A: Chỉ đổi thứ tự (DisplayOrder) giữa các step **cùng cấp** (cùng
  ParentId) — giống hệt phạm vi của Move Up/Down hiện có. Kéo thả KHÔNG cho phép đổi ParentId
  (không hỗ trợ "thả để làm con" của step khác) trong phạm vi tính năng này.
- Change: TemplateBuilderPage MUST bổ sung khả năng kéo thả (drag-and-drop) trên từng dòng step
  trong cây bước để sắp xếp lại thứ tự — **bổ sung thêm bên cạnh** 2 nút Move Up/Down hiện có
  (FR-006), KHÔNG thay thế hay gỡ bỏ 2 nút này. Người dùng có thể dùng cách nào tùy ý (kéo thả hoặc
  bấm nút) để đạt cùng một kết quả sắp xếp.
- Change: Kéo thả chỉ khả dụng giữa các step cùng cấp (cùng ParentId) với step đang kéo; thả vào vị
  trí không cùng cấp (khác nhánh cha/con) MUST không tạo ra thay đổi ParentId nào — tính năng này
  không hỗ trợ "reparent" qua kéo thả.
- Change: Sau khi thả vào một vị trí hợp lệ, DisplayOrder của toàn bộ nhóm anh em bị ảnh hưởng MUST
  được cập nhật ngay trên giao diện (dùng chung cơ chế `reorderSiblings` với Move Up/Down), đánh dấu
  màn hình có thay đổi chưa lưu, và chỉ thực sự ghi xuống `eutr_template_details` khi người dùng
  nhấn Save — không tự động lưu ngay khi kéo thả.
- Change: Kéo thả chỉ khả dụng khi template đang Status=Draft; khi Status=Approved (read-only theo
  FR-061), thao tác kéo thả MUST bị vô hiệu hóa giống các thao tác chỉnh sửa cây bước khác (Add/Edit/
  Delete step, Move Up/Down).

### Session 2026-07-23 (Update 18) — Cho phép chỉnh sửa "Set as default template" khi Approved

- Input: "cập nhật 003-eutr-templates. với template trạng thái approve, cho chỉnh sửa Set as
  default template."
- Q: Checkbox "Set as default template" trên TemplateBuilderPage được mở khóa khi Status=Approved
  (khác các trường header khác vẫn read-only theo FR-061). Nút Save hiện đang ẩn/disabled hoàn toàn
  khi Approved (FR-061) — vậy khi user tick/bỏ tick checkbox này, thay đổi được lưu xuống DB bằng
  cách nào? → A: Auto-save kèm `ConfirmDialog` Yes/No — tick/bỏ tick checkbox hiển thị hộp thoại xác
  nhận Yes/No (theo đúng pattern Approve/Request change đã có ở FR-059/FR-060); chọn Yes mới thực sự
  gọi API cập nhật ngay lập tức (chỉ đổi cột IsDefault, không đụng đến Name/AlertFor/step tree/
  Version), áp dụng luôn ràng buộc toàn cục FR-040 (tự bỏ cờ default trên template khác đang là
  default, nếu có); chọn No đóng hộp thoại và checkbox quay lại giá trị trước đó (không lưu gì).
- Change: **Đảo ngược một phần FR-061** — checkbox **Set as default template** trên
  TemplateBuilderPage KHÔNG còn bị khóa (disabled) khi Status=Approved như quy định trước đây; đây
  là trường DUY NHẤT trên header vẫn có thể chỉnh sửa được ở chế độ read-only. Toàn bộ phần còn lại
  của FR-061 (Name, Alert for disabled, Save ẩn/disabled, mọi thao tác trên cây bước bị khóa) giữ
  nguyên không đổi.
- Change: Thêm cơ chế lưu riêng cho checkbox Set as default khi Approved — độc lập với luồng Save
  chính của TemplateBuilderPage (vốn chỉ khả dụng khi Draft) — xem FR-068.

### Session 2026-07-24 (Update 19) — TakeFrom lấy từ `eutr_reference_types` + TemplateListPage đổi sang DataGridStyled

- Input: "cập nhật 003-eutr-templates thông tin take from lấy từ bảng eutr_reference_types và
  TableContainer ở TemplateListPage sửa lại thành DataGridStyled."
- Q: Combobox/tra cứu **TakeFrom** hiện đang dùng 2 hằng số cứng trong
  `compliance-client/src/utils/helpers.js` (`TAKE_FROM_OPTIONS` cho combobox, `TAKE_FROM_LABELS`
  cho tra cứu label hiển thị) — đổi nguồn dữ liệu này áp dụng cho TẤT CẢ nơi đang dùng 2 hằng số đó
  (Add step, Edit step, Bulk Add Steps dialog, hiển thị label trên cây bước ở TemplateBuilderPage),
  hay chỉ một phần? → A: Áp dụng cho TẤT CẢ — mọi combobox chọn TakeFrom và mọi nơi tra cứu label
  TakeFrom để hiển thị MUST chuyển sang lấy dữ liệu từ bảng `eutr_reference_types` (qua API sẵn có,
  không thêm bảng/endpoint mới), thay thế hoàn toàn `TAKE_FROM_OPTIONS`/`TAKE_FROM_LABELS` cứng.
  `REQUIREMENT_TYPES`/`REQUIREMENT_LABELS` (Required/Optional) KHÔNG nằm trong phạm vi thay đổi này
  — vẫn giữ nguyên là hằng số cứng như hiện tại.
- Q: Việc đổi **TableContainer** ở TemplateListPage sang **DataGridStyled** có kèm đổi lại bộ cột/
  tính năng hiển thị (quay về 9 cột DataGrid kiểu Update 9, hay bật lại sort/filter/column-visibility
  theo cột) hay chỉ đổi công nghệ hiển thị (container), giữ nguyên toàn bộ nội dung/tính năng hiện
  có của Update 10-18 (ô tên 2 dòng Code/Name, Chip Status/Version/Default, cột Steps, ô tìm kiếm
  server-side, checkbox chọn dòng, 4 icon Action)? → A: Chỉ đổi công nghệ hiển thị (container) —
  TemplateListPage tiếp tục dùng component dùng chung `DataGridStyled`
  (`@presentation/components/shared/DataGridStyles`) bọc MUI `DataGrid` (đúng pattern các trang
  danh sách EUTR khác đang dùng, ví dụ `eutr-reference-types/index.jsx`, `eutr-steps/index.jsx`),
  thay cho `TableContainer`/`Table`/`TableHead`/`TableBody`/`TableRow`/`TableCell`/`TablePagination`
  hiện tại. Toàn bộ nội dung/tính năng mỗi dòng và mỗi cột GIỮ NGUYÊN không đổi — chỉ là kỹ thuật
  render khác (dùng `columns` + `renderCell` của DataGrid thay vì JSX Table thủ công). KHÔNG bật lại
  sort/filter/column-visibility theo cột hay Import/Export (các tính năng này vẫn hoãn lại theo
  FR-021b, không thay đổi ở đợt này).
- Change: Combobox **TakeFrom** ở form Add step (FR-007), form Edit step (FR-008b), và bảng
  bulk-select trong dialog **Add Root Group/Add Child Step** (FR-027 đến FR-030,
  `BulkAddStepsDialog.jsx`) MUST tải danh sách lựa chọn từ bảng `eutr_reference_types` qua API
  `GET /api/eutr-reference-types` (endpoint có sẵn từ feature 006-eutr-reference-types, KHÔNG thêm
  endpoint mới), hiển thị cột `Name` làm label và dùng `Id` làm value — thay cho hằng số cứng
  `TAKE_FROM_OPTIONS` hiện tại trong `helpers.js`. Giá trị lưu vào cột `TakeFrom` của bảng
  `eutr_template_details` vẫn là `Id` (khớp đúng ràng buộc khóa ngoại
  `eutr_template_details_takefrom_foreign` → `eutr_reference_types.Id` đã có sẵn trong
  `docs/design/eutr/eutr_db.sql`) — không đổi kiểu dữ liệu cột.
  Xem FR-072, FR-073.
- Change: Mọi nơi đang tra cứu **nhãn hiển thị** của TakeFrom theo `Id` (ví dụ hiển thị tên TakeFrom
  trên từng dòng của cây bước ở `StepTree.jsx`/`TemplateBuilderPage.jsx`, đang dùng map cứng
  `TAKE_FROM_LABELS`) MUST đổi sang tra cứu `Name` từ cùng danh sách `eutr_reference_types` đã tải
  (theo `Id` đang lưu ở `TakeFrom`), thay vì map cứng. Nếu `Id` không còn tồn tại trong
  `eutr_reference_types` (đã bị xóa), nhãn hiển thị để trống thay vì lỗi. Xem FR-073.
- Change: Hằng số `TAKE_FROM_OPTIONS` và `TAKE_FROM_LABELS` trong
  `compliance-client/src/utils/helpers.js` (thêm ở Update 8) bị loại bỏ khỏi vai trò nguồn dữ liệu
  chính cho TakeFrom — các component MUST gọi API `GET /api/eutr-reference-types` (qua use
  case/hook tương ứng, ví dụ tái sử dụng `GetEutrReferenceTypesUseCase`/`repositories.eutrReferenceTypes`
  đã có ở feature 006) và tự xây danh sách options/label từ kết quả trả về. `REQUIREMENT_TYPES`/
  `REQUIREMENT_LABELS` không bị ảnh hưởng.
- Change: Component **TemplateListPage**
  (`compliance-client/src/presentation/pages/eutr-templates/TemplateListPage.jsx`) thay toàn bộ
  phần hiển thị bảng đang dùng `TableContainer`/`Table`/`TableHead`/`TableRow`/`TableCell`/
  `TableBody`/`TablePagination` (Update 10) bằng `DataGridStyled` bọc MUI `DataGrid`, theo đúng
  pattern dùng chung của các trang danh sách EUTR khác trong hệ thống (`DataGridStyled` từ
  `@presentation/components/shared/DataGridStyles`, kết hợp `useDynamicGridHeight` nếu phù hợp).
  Toàn bộ nội dung/tính năng hiện có của FR-021/FR-021a/FR-021b/FR-021c/FR-022 GIỮ NGUYÊN, chỉ đổi
  cách render — xem FR-069 đến FR-071.
- Change: Ô tên 2 dòng (Code đậm/Name phụ), Chip Status, Chip Version, Chip Default, số Steps, và
  4 icon Action (Edit/Clone/Apply to Customer/Delete) MUST tiếp tục hiển thị đúng như hiện tại,
  triển khai qua `renderCell` của từng cột DataGrid tương ứng thay vì `TableCell` thủ công.
- Change: Checkbox chọn dòng để xóa hàng loạt (FR-021, bulk delete) MUST dùng cơ chế
  `checkboxSelection`/`onRowSelectionModelChange` sẵn có của MUI `DataGrid` thay cho `Checkbox`
  thủ công trong từng `TableCell`; hành vi chọn/bỏ chọn tất cả, xóa hàng loạt (FR-021, phần bulk
  delete đã mô tả ở Update 10) không đổi.
- Change: Phân trang MUST dùng cơ chế phân trang tích hợp sẵn của `DataGrid`
  (`pagination`, `paginationMode="server"`, `paginationModel`/`onPaginationModelChange`,
  `rowCount={total}`) thay cho `TablePagination` thủ công hiện tại — hành vi chuyển trang/đổi số
  dòng mỗi trang không đổi so với hiện tại.
- Change: Ô tìm kiếm Code/Name (FR-021a, server-side, đặt phía trên bảng) GIỮ NGUYÊN vị trí và hành
  vi — nằm ngoài `DataGridStyled`/`DataGrid`, không đổi.

### Session 2026-07-24 (Update 20) — Bật filter theo cột trên TemplateListPage (giống country-groups/index.jsx)

- Input: "thêm filter các cột giống E:\Working\Eutr\compliance-client\src\presentation\pages\country-groups\index.jsx".
- Change: TemplateListPage MUST bật khả năng filter theo từng cột kiểu MUI `DataGrid` (mở panel
  filter qua menu 3 chấm trên header cột — cùng cơ chế `filterMode="server"`/`filterModel`/
  `onFilterModelChange` mà `country-groups/index.jsx` đang dùng), CHỈ cho các cột có ánh xạ lọc thật
  ở backend: **Template Name** (lọc theo Name), **Status**, **Version**, **Default**. Cột **Steps**
  (số tính từ subquery) và cột **Actions** KHÔNG bật filter — cùng cách `country-groups/index.jsx`
  không bật filter cho cột tính toán `memberCount`/`countryCodes` của chính nó.
- Change: Ô tìm kiếm nhanh Code/Name hiện có (FR-021a) MUST tiếp tục hoạt động song song với filter
  theo cột mới — 2 cơ chế cùng ghi vào `filterModel` nhưng KHÔNG được ghi đè lẫn nhau: đổi ô tìm
  kiếm chỉ thay phần tử `keyword` trong `filterModel.items`, giữ nguyên các filter cột khác đang có;
  ngược lại, sửa filter cột qua panel của DataGrid MUST giữ lại phần tử `keyword` (nếu đang có) thay
  vì làm mất nó.
- Change: Backend (`EutrTemplatesRepository.FilterMap`, whitelist cột được phép lọc) MUST mở rộng
  thêm `Status`, `VersionId`, `IsDefault` (đã có sẵn `Code`, `Name`, `AlertFor`, `CreatedBy`) — cả 3
  cột này đã có trong câu SELECT hiện tại, chỉ thiếu whitelist filter. KHÔNG đổi API
  contract/response shape, KHÔNG thêm endpoint mới.
...
**TemplateListPage** với nội dung/bố cục theo đúng thiết kế tham chiếu `TemplateListPage.jsx`
(Update 10 — đảo ngược quyết định giữ DataGrid ở Update 9), nhưng từ Update 19 được render bằng
`DataGridStyled`/MUI `DataGrid` thay cho `TableContainer`/`Table` thuần, cộng với ô tìm kiếm phía
trên. Mỗi dòng hiển thị: **Code** (chữ đậm, dòng trên), **Name** (chữ phụ/caption, dòng dưới), Chip
**Version**
...
bởi Update 12 (xem FR-008b, FR-031). Người dùng có thể sắp xếp lại thứ tự (DisplayOrder) các step
cùng cấp bằng hai cách tương đương: bấm nút **Move Up/Move Down** trên toolbar (FR-006), hoặc
**(Update 17)** kéo thả (drag-and-drop) trực tiếp một dòng step tới vị trí khác trong cùng cấp —
xem FR-064 đến FR-066. Toàn bộ màn hình Edit này (header + step tree) chỉ khả dụng
khi template đang Status=**Draft**. Khi nhấn Save (Draft), hệ thống MUST cập nhật đè trực tiếp lên
dòng hiện tại (cùng Id, cùng VersionId, CreatedDate không đổi) — **(Superseded by Update 16 — xem
FR-057)** logic cũ dựa trên tuổi bản ghi (24 giờ, mô tả ở 2 gạch đầu dòng bên dưới) KHÔNG còn áp
dụng, giữ lại chỉ để tham khảo lịch sử:
...
1. **(Superseded by Update 16 — xem 1c, FR-057)** ~~Given template "T001" đang ở VersionId=1 và
   được tạo cách đây TRÊN 24 giờ, When nhấn Edit, chỉnh sửa rồi Save, Then hệ thống tạo dòng mới
   với cùng Code nhưng VersionId=2, dòng cũ cập nhật IsHide=1.~~
1a. **(Superseded by Update 16 — xem 1c)** ~~Given template "T002" đang ở VersionId=1 và được tạo
   cách đây DƯỚI 24 giờ, When nhấn Edit, chỉnh sửa Name và Save, Then hệ thống cập nhật đè lên
   dòng hiện tại: cùng Id, cùng VersionId=1, CreatedDate không đổi, Name mới được lưu, không có
   dòng mới nào được tạo.~~
1b. **(Superseded by Update 16 — xem 1c)** ~~Given template "T002" vừa được cập nhật đè (VersionId
   vẫn = 1, dưới 24h kể từ tạo), When người dùng Edit lần nữa ngay sau đó (vẫn dưới 24h kể từ
   CreatedDate gốc) và Save, Then hệ thống tiếp tục cập nhật đè lên cùng dòng đó (không tạo version
   mới).~~
1c. **(Update 16)** **Given** template "T001" đang Status=Draft, VersionId=1 (bất kể CreatedDate đã
   qua bao lâu), **When** nhấn Edit, chỉnh sửa Name và/hoặc step tree rồi Save, **Then** hệ thống
...
3. **Given** đang edit template (Status=Draft), **When** xóa một step khỏi cây, lưu, **Then** dòng
   hiện tại (cùng Id/VersionId) chứa cây bước đã bỏ step đó — không có dòng nào khác bị ảnh hưởng.
4. **Given** template có nhiều version (1, 2, 3) do đã trải qua nhiều lần Request change, **When**
   xem grid, **Then** chỉ hiển thị version mới nhất (IsHide=0).
5. **Given** đang edit template (Draft) có step "Forest" với Required + PO, **When** nhấn icon Edit
...
7b. **Given** đang edit template (Draft), **When** đổi Alert for sang group khác rồi Save, **Then**
   Id của group mới được lưu vào AlertFor, đè trực tiếp lên dòng hiện tại (Update 16 — không còn
   phân nhánh 24 giờ).
8. **Given** đang edit template (Draft), **When** thêm một step mới bằng cách gõ tự do một tên chưa
   có trong danh sách EUTR steps rồi Save step, sau đó Save template, **Then** hệ thống tạo bản ghi
   step mới trong eutr_steps và lưu step đó vào eutr_template_details với StepId vừa tạo, đè trực
   tiếp lên dòng hiện tại (Update 16 — không còn phân nhánh 24 giờ).
9. **Given** một template vừa được tạo qua dialog quick-create (0 step, Vendor trống, Status=Draft),
   **When** nhấn Edit trên dòng đó, **Then** màn hình TemplateBuilderPage hiển thị cây bước rỗng
   (trạng thái "chưa có step nào") và panel cấu hình có combobox Vendor chưa chọn giá trị nào, cho
   phép người dùng thêm step đầu tiên và chọn Vendor.
10. **Given** đang ở màn hình Edit của một template (Draft) chưa có Vendor, **When** chọn một
    vendor từ combobox Vendor rồi Save, **Then** cột VendorCode của template được cập nhật đè trực
    tiếp lên dòng hiện tại (Update 16 — không còn phân nhánh 24 giờ).
...
16. **(Update 17)** **Given** đang edit template (Draft) có 3 step cùng cấp A, B, C theo thứ tự
    ...
    kết quả nếu dùng nút Move Up/Move Down để đạt cùng vị trí — thay đổi này CHƯA được ghi xuống
    `eutr_template_details` cho đến khi nhấn Save.
17. **(Update 17)** **Given** đang edit template (Draft), **When** người dùng cố kéo một step thả
...
- Khi nhấn Back trên màn hình tạo/sửa mà đã add/edit step nhưng chưa Save, hệ thống hiển thị cảnh
  ...
  header) sẽ bị mất.
- **(Superseded by Update 16)** ~~Khi Edit một template được tạo cách đây đúng 24 giờ (biên giới),
  hệ thống áp dụng logic dựa trên so sánh (now - CreatedDate) — nếu ≥ 24 giờ thì tạo version mới,
  nếu < 24 giờ thì cập nhật đè.~~ ~~Khi một template được cập nhật đè nhiều lần liên tiếp trong
  cùng khung 24 giờ kể từ CreatedDate gốc, VersionId và Id không đổi qua các lần Save đó; chỉ khi
...
- **FR-006 (Cập nhật ở Update 17 — xem FR-064 đến FR-066)**: Màn hình Edit MUST hiển thị cây bước
  (step tree) đệ quy, hỗ trợ collapse/expand từng nhánh và sắp xếp lại thứ tự step trong cùng cấp
  bằng 2 nút **Move Up/Move Down** trên toolbar (thao tác hiện có, đã triển khai). DisplayOrder MUST
  được cập nhật tự động theo vị trí mới. Chức năng này CHỈ khả dụng ở màn hình Edit, không có ở
  dialog Create Template. *(Update 17 bổ sung thêm một cách thứ hai để đạt cùng kết quả — kéo thả
  (drag-and-drop) — xem FR-064.)*
- **FR-007 (nguồn dữ liệu TakeFrom đổi ở Update 19 — xem FR-072)**: Khi nhấn Add step (chỉ khả
  dụng ở màn hình Edit), hệ thống MUST hiển thị form chọn step gồm: combobox step (free-solo — nạp
  danh sách từ EUTR steps hiện có, cho phép chọn 1 step có sẵn HOẶC gõ trực tiếp một tên step mới
  chưa có trong danh sách), combobox RequirementType (Required/Optional), combobox TakeFrom
  (nạp động từ bảng `eutr_reference_types` — xem FR-072, không còn là danh sách cứng "PO/Upload
  manual"), và nút Save.
...
- **FR-008b (nguồn dữ liệu TakeFrom đổi ở Update 19 — xem FR-072)**: Người dùng MUST có thể chỉnh
  ...
  mới chưa có trong danh sách), combobox RequirementType (Required/Optional) với giá trị hiện tại
  được chọn sẵn, combobox TakeFrom (nạp động từ `eutr_reference_types` — xem FR-072) với giá trị
  hiện tại được chọn sẵn, nút Save (xác nhận thay đổi) và nút Cancel (hủy, giữ giá
...
- **FR-009 (Cập nhật ở Update 16)**: Khi nhấn Save trong dialog **Create Template**, hệ thống MUST
  tạo một bản ghi mới trong eutr_templates (Code tự sinh, Name, IsDefault, AlertFor, VersionId=1,
  Status=**Draft** — Update 16, xem FR-056, IsDeleted=0, IsHide=0) và KHÔNG tạo step nào trong
  eutr_template_details (cây bước rỗng ban đầu). Việc lưu cây bước (bao gồm ParentId chính xác cho
  từng step) chỉ MUST xảy ra khi Save ở màn hình **Edit** (chỉ khả dụng khi Status=Draft — theo
  FR-057, Update 16 đã bỏ cơ chế versioning theo tuổi bản ghi của FR-012).
...
- **FR-011 (Superseded by Update 10 — xem FR-023)**: Khi nhấn Edit trên một dòng (bao gồm cả
  ...
  đổi Vendor.
- **FR-012 (Superseded by Update 16 — xem FR-057)**: Khi Save ở màn hình Edit, hệ thống MUST áp
  dụng logic versioning có điều kiện dựa trên CreatedDate của bản ghi đang sửa so với thời điểm
  hiện tại:
...
- **FR-021 (Cập nhật ở Update 16 — thêm Status; render đổi ở Update 19 — xem FR-069)**: Giao diện
  danh sách **TemplateListPage** MUST hiển thị nội dung sau kèm ô tìm kiếm phía trên, theo đúng bố
  cục của thiết kế tham chiếu `TemplateListPage.jsx`. *(Update 10 quy định render bằng Table thuần;
  Update 19 — 2026-07-24 — đổi sang render bằng `DataGridStyled`/MUI `DataGrid`, xem FR-069 đến
...
- **FR-021b (một phần Superseded ở Update 20 — xem FR-074/FR-075)**: Các tính năng sau của
  TemplateListPageOld KHÔNG được mang sang giao diện Table mới ở đợt cập nhật này (hoãn lại —
  deferred): nút Import/Export trên toolbar, ẩn/hiện cột (column visibility), và filter/sort theo
  từng cột kiểu DataGrid. Cột Vendor code, Vendor name, Alert for, Created by, Created date KHÔNG
  còn hiển thị trực tiếp trên danh sách (vẫn xem/sửa được ở TemplateBuilderPage theo FR-024).
  *(Update 20 bật lại filter theo cột — KHÔNG bật sort/column-visibility/Import-Export, các mục này
  vẫn hoãn lại như cũ — xem FR-074/FR-075.)*
...
- **FR-023**: Icon **Edit** trên mỗi dòng của TemplateListPage MUST điều hướng tới route
  ...
  Vendor trống).
- **FR-024 (Cập nhật ở Update 13 và Update 16 — xem FR-041, FR-057, FR-061)**: **TemplateBuilderPage**
  MUST được nối với dữ liệu và luồng nghiệp vụ thật, tái sử dụng toàn bộ logic nghiệp vụ đã có ở
  ...
  FR-041, Update 13); đây là màn hình duy nhất người dùng chỉnh sửa các trường này (chỉ khi
  Status=Draft).
...
- **FR-049 (Update 15; trigger cập nhật ở Update 16 — xem FR-060)**: Khi hệ thống lên version cho
  một template (tạo dòng mới trong eutr_templates với VersionId+1 và TemplateId mới, dòng cũ
  IsHide=1), hệ thống MUST đồng thời sao chép toàn bộ bản ghi hiện có trong
  `eutr_template_references` của TemplateId cũ sang TemplateId mới, giữ nguyên
  VendorCode/FromDate/ToDate/CreatedBy/CreatedDate của từng mapping. *(Update 16: thời điểm lên
  version không còn là nhánh "trên 24 giờ" của FR-012 (đã bỏ) mà là lúc người dùng xác nhận
  **Request change** để chuyển template từ Approved về Draft — xem FR-060. Logic sao chép mapping
  ở trên giữ nguyên không đổi, chỉ đổi thời điểm kích hoạt.)* Các bản ghi mapping cũ (gắn với
  TemplateId cũ, đã IsHide=1) MUST được giữ nguyên, không xóa hay di chuyển.
...
- **FR-053 (Update 15; cập nhật ở Update 16)**: Sau khi người dùng đồng ý ở hộp thoại xác nhận
  (FR-052), hệ thống MUST thực hiện đồng thời: (1) tạo một bản ghi mới trong eutr_templates với
  Code tự sinh mới (theo quy tắc hiện hành), Name = giá trị người dùng nhập, AlertFor = Id group đã
  chọn, VersionId=1, Status=**Draft** (Update 16, xem FR-056), IsDefault=0 (luôn = 0 bất kể template
  nguồn), IsDeleted=0, IsHide=0; (2) sao chép toàn bộ cây bước
...
- **FR-056 (Update 16)**: Khi tạo template mới — qua dialog **Create Template** (FR-009) hoặc qua
  **Clone** (FR-053) — hệ thống MUST đặt Status = **Draft** mặc định cho bản ghi mới tạo.
- **FR-057 (Update 16 — thay thế FR-012)**: Hệ thống MUST bỏ hoàn toàn nhánh versioning dựa trên
  tuổi bản ghi (24 giờ) của FR-012. Khi Save ở TemplateBuilderPage (chỉ khả dụng khi Status đang là
  Draft — xem FR-061), hệ thống MUST LUÔN cập nhật đè trực tiếp lên dòng hiện tại (cùng Id, cùng
  VersionId, CreatedDate không đổi) — không tạo dòng mới, không phân biệt CreatedDate đã qua bao
...
- **FR-064 (Update 17)**: TemplateBuilderPage MUST bổ sung khả năng kéo thả (drag-and-drop) trên
  ...
  bỏ 2 nút này. Người dùng có thể dùng cách nào tùy ý (kéo thả hoặc bấm nút) để đạt cùng một kết quả
  sắp xếp.
- **FR-065 (Update 17)**: Kéo thả một step MUST chỉ cho phép thả vào vị trí giữa các step cùng
  ParentId với step đang kéo. Thả một step vào vị trí không cùng cấp (ví dụ lên một step ở nhánh
  cha/con khác) MUST KHÔNG làm thay đổi ParentId của step đó — hệ thống KHÔNG hỗ trợ đổi cấp
  (reparent) qua kéo thả trong phạm vi tính năng này.
- **FR-066 (Update 17)**: Sau khi kéo thả hoàn tất tại một vị trí hợp lệ (cùng cấp), DisplayOrder
...
- **FR-069 (Update 19)**: **TemplateListPage** MUST render bảng danh sách bằng component dùng chung
  ...
  `eutr-steps/index.jsx`), thay thế hoàn toàn `TableContainer`/`Table`/`TableHead`/`TableRow`/
  `TableCell`/`TableBody`/`TablePagination` hiện tại (Update 10). Đây CHỈ là thay đổi kỹ thuật
  render — toàn bộ nội dung/tính năng mỗi cột mô tả ở FR-021 (ô tên 2 dòng Code/Name, Chip Status/
  Version/Default, cột Steps, cột Actions) GIỮ NGUYÊN không đổi.
- **FR-070 (Update 19)**: Mỗi cột hiện có của TemplateListPage MUST được triển khai lại dưới dạng
  cột `DataGrid` (`columns` + `renderCell`) tương ứng: cột tên hiển thị Code (đậm)/Name (caption)
  qua `renderCell`; cột Status/Version/Default hiển thị Chip qua `renderCell` (giữ đúng điều kiện
  hiển thị hiện có — Default chỉ hiện khi IsDefault=1); cột Steps hiển thị số lượng step; cột
  Actions hiển thị 4 icon Edit/Clone/Apply to Customer/Delete qua `renderCell`, giữ nguyên trạng
  thái enabled/disabled và hành vi onClick hiện có (FR-021, FR-026, FR-050, FR-032). Phân trang
  MUST dùng cơ chế tích hợp sẵn của `DataGrid` (`pagination`, `paginationMode="server"`,
  `paginationModel`/`onPaginationModelChange`, `rowCount={total}`) thay cho `TablePagination` thủ
  công, tái sử dụng nguyên `paginationModel`/`setPaginationModel` đã có ở `useEutrTemplatesData`.
  Sort/filter theo từng cột kiểu DataGrid và Import/Export vẫn KHÔNG được bật (giữ nguyên theo
  FR-021b — hoãn lại).
- **FR-071 (Update 19)**: Checkbox chọn dòng phục vụ xóa hàng loạt (mô tả tại FR-021/Update 10)
  MUST dùng cơ chế `checkboxSelection`/`onRowSelectionModelChange` tích hợp sẵn của MUI `DataGrid`
  thay cho `Checkbox` thủ công trong từng ô; hành vi chọn/bỏ chọn tất cả và xóa hàng loạt (gọi
  `DeleteMultiEutrTemplatesUseCase`) không đổi. Ô tìm kiếm Code/Name (FR-021a) GIỮ NGUYÊN vị trí và
  hành vi, nằm ngoài `DataGridStyled`/`DataGrid`.
- **FR-072 (Update 19)**: Combobox **TakeFrom** ở form Add step (FR-007), form Edit step (FR-008b),
  và bảng bulk-select trong dialog Add Root Group/Add Child Step (FR-027 đến FR-030,
  `BulkAddStepsDialog.jsx`) MUST tải danh sách lựa chọn từ bảng `eutr_reference_types` qua API sẵn
  có `GET /api/eutr-reference-types` (feature 006-eutr-reference-types, KHÔNG thêm endpoint mới),
  hiển thị cột `Name` làm label và dùng `Id` làm value, thay cho hằng số cứng `TAKE_FROM_OPTIONS`
  trong `compliance-client/src/utils/helpers.js`. Giá trị lưu vào cột `TakeFrom` của
  `eutr_template_details` vẫn là `Id` — khớp đúng khóa ngoại
  `eutr_template_details_takefrom_foreign` → `eutr_reference_types.Id` đã có sẵn trong
  `docs/design/eutr/eutr_db.sql`, không đổi kiểu dữ liệu cột. `REQUIREMENT_TYPES` (Required/Optional)
  KHÔNG bị ảnh hưởng, vẫn là hằng số cứng như hiện tại.
- **FR-073 (Update 19)**: Mọi nơi đang tra cứu **nhãn hiển thị** của TakeFrom theo `Id` để hiển thị
  trên cây bước (`StepTree.jsx`/`TemplateBuilderPage.jsx`, hiện dùng map cứng `TAKE_FROM_LABELS`)
  MUST đổi sang tra cứu `Name` từ cùng danh sách `eutr_reference_types` đã tải theo FR-072 (khớp
  theo `Id` đang lưu ở `TakeFrom`), thay vì map cứng. Nếu `Id` không còn tồn tại trong
  `eutr_reference_types` (đã bị xóa), nhãn hiển thị để trống thay vì gây lỗi hiển thị.
  `TAKE_FROM_OPTIONS`/`TAKE_FROM_LABELS` trong `helpers.js` không còn là nguồn dữ liệu chính cho
  TakeFrom sau thay đổi này.
- **FR-074 (Update 20)**: TemplateListPage MUST bật filter theo cột (menu 3 chấm trên header cột
  của MUI `DataGrid`, `filterMode="server"`) cho các cột **Template Name** (lọc theo Name),
  **Status**, **Version**, **Default** — theo đúng cơ chế `country-groups/index.jsx` đang dùng
  (`filterModel`/`onFilterModelChange` liên thông với `useEutrTemplatesData`). Cột **Steps** và
  **Actions** KHÔNG bật filter (số tính từ subquery / cột thao tác, không có ánh xạ lọc ở backend).
  Sort theo cột và ẩn/hiện cột (column visibility) KHÔNG được bật thêm — vẫn hoãn lại theo FR-021b.
- **FR-075 (Update 20)**: Ô tìm kiếm nhanh Code/Name (FR-021a) và filter theo cột (FR-074) MUST
  cùng tồn tại mà không ghi đè lẫn nhau trên `filterModel`: đổi ô tìm kiếm chỉ thay/xóa phần tử
  `keyword`, giữ nguyên các filter cột khác; sửa filter cột qua panel của DataGrid MUST giữ lại
  phần tử `keyword` đang có (nếu còn giá trị) thay vì làm mất nó. Backend
...
- **EUTR Template** *(Update 13: không còn gắn trực tiếp với vendor — xem EUTR Template
  ...
  trực tiếp lên dòng hiện tại (giữ nguyên Id, VersionId, CreatedDate).~~ **(Update 16)** Khi Save ở
...
- **EUTR Template Detail**: Đại diện cho một bước cụ thể trong cây bước của template. Thuộc tính:
  ...
  TakeFrom (khóa ngoại `Id` tới bảng `eutr_reference_types` — **Update 19**, ràng buộc
  `eutr_template_details_takefrom_foreign`; KHÔNG còn là enum cứng 2 giá trị PO/Upload manual như
  trước), thứ tự hiển thị, người tạo, ngày tạo. Khi edit template, toàn bộ cây bước hiện tại (bao
  gồm step đã chỉnh sửa) được lưu vào TemplateId mới. ParentId MUST được lưu chính xác để duy trì
  cấu trúc cây đệ quy. Trên frontend, hằng số `REQUIREMENT_TYPES`/`REQUIREMENT_LABELS` (mảng
  options `{value, label}[]` cho combobox và map tra cứu label theo value) vẫn được khai báo dùng
  chung tại `compliance-client/src/utils/helpers.js` để `StepTree.jsx`, `StepFormRow.jsx` và các
  chức năng khác tái sử dụng — nhưng `TAKE_FROM_OPTIONS`/`TAKE_FROM_LABELS` (cùng khai báo cứng
  trước đây) **(Update 19)** không còn là nguồn dữ liệu cho TakeFrom; combobox và tra cứu nhãn
  TakeFrom MUST tải động từ bảng `eutr_reference_types` (xem FR-072, FR-073, Key Entity **EUTR
  Reference Type** bên dưới). **(Update 15)** Khi Clone một template, toàn bộ
...
- **EUTR Template Reference** *(mới — Update 13, bảng `eutr_template_references`)*: Đại diện cho
  ...
  **(Update 14)** Có thể Export hàng loạt các bản ghi này (thuộc một TemplateId) ra file Excel
  ...
  không thuộc phạm vi kiểm tra chồng lấn của FR-036).
...
- **EUTR Reference Type** *(mới đưa vào phạm vi feature này ở Update 19 — bảng `eutr_reference_types`,
  quản lý CRUD ở feature riêng 006-eutr-reference-types)*: Danh mục loại tham chiếu dùng chung của
  hệ thống. Thuộc tính liên quan: `Id`, `Name`, người tạo/ngày tạo, người cập nhật/ngày cập nhật.
  **(Update 19)** Combobox **TakeFrom** ở form Add step (FR-007), form Edit step (FR-008b), bảng
  bulk-select trong dialog Add Root Group/Add Child Step (FR-027 đến FR-030), và mọi nơi tra cứu
  nhãn hiển thị TakeFrom trên cây bước (StepTree.jsx/TemplateBuilderPage.jsx) MUST lấy dữ liệu từ
  bảng này qua API sẵn có `GET /api/eutr-reference-types` (feature 006, KHÔNG thêm endpoint mới),
...
- **SC-007 (Superseded by Update 16 — xem SC-045, SC-046, SC-048)**: Sau khi edit và save một
  template được tạo cách đây TRÊN 24 giờ, phiên bản cũ (IsHide=1) vẫn tồn tại trong database và
  phiên bản mới (VersionId tăng) hiển thị đúng trên grid. Sau khi edit và save một template được
  tạo cách đây DƯỚI 24 giờ, KHÔNG có dòng mới nào được tạo — dữ liệu được cập nhật đè lên dòng hiện
  có (VersionId và Id không đổi).
...
- **SC-013 (Superseded by Update 16 — xem SC-046)**: 100% lượt Edit một template được tạo dưới 24
  giờ trước đó dẫn đến cập nhật đè lên dòng hiện tại (không tăng VersionId, không tạo dòng ẩn mới).
  100% lượt Edit một template được tạo trên 24 giờ trước đó dẫn đến tạo version mới đúng theo cơ
  chế versioning hiện hành.
...
- **SC-040 (Update 15; trigger cập nhật ở Update 16 — xem SC-048)**: 100% lượt lên version (nay là
  khi xác nhận Request change, không còn là nhánh "trên 24 giờ" của FR-012 đã bỏ) sao chép đầy đủ
  và chính xác toàn bộ mapping vendor (`eutr_template_references`) từ TemplateId cũ sang TemplateId
  mới — không có mapping nào bị thiếu hoặc sai lệch sau khi xác nhận.
...
- **SC-045 (Update 16)**: 100% template mới tạo (qua Create Template hoặc Clone) có Status=Draft
  ngay sau khi tạo.
...
- **SC-053 (Update 19)**: 100% combobox TakeFrom (Add step, Edit step, bulk-select) hiển thị đúng
  ...
  form kế tiếp, không cần sửa code. 0% trường hợp danh sách TakeFrom còn hiển thị theo hằng số cứng
  cũ (PO/Vendor/Invoice/Delivery note/General agreement) sau khi hoàn tất thay đổi.
- **SC-054 (Update 19)**: TemplateListPage sau khi đổi sang `DataGridStyled`/`DataGrid` hiển thị
  100% đúng dữ liệu và tính năng như trước khi đổi (ô tên 2 dòng, Chip Status/Version/Default, số
  Steps, 4 icon Actions, tìm kiếm server-side, phân trang, chọn dòng xóa hàng loạt) — không phát
  sinh khác biệt hành vi nào ngoài công nghệ render.
- **SC-055 (Update 20)**: Người dùng filter theo cột Template Name/Status/Version/Default trên
...
- **(Superseded by Update 16)** ~~Edit template sử dụng cơ chế versioning có điều kiện theo tuổi
  ...
  cũ mà tạo dòng mới với VersionId+1, ẩn dòng cũ (IsHide=1), lưu template_details sang Id mới; nếu
...
- **(Update 13)** Route cho màn hình ApplyCustomerPage đề xuất là `/eutr/templates/apply/:id`, theo
  ...
  kiện khóa theo `status`/`Published` của mockup (tại thời điểm Update 13, EUTR Template thật chưa
  có khái niệm Status; **Update 16** sau đó bổ sung lại Status thật — Draft/Approved — nhưng
  KHÔNG dùng để khóa Apply to Customer: ApplyCustomerPage vẫn hoạt động ở bất kỳ Status nào của
  template, theo đúng FR-032), và lưu/tải qua backend CRUD mới cho `eutr_template_references` thay
  vì state cục bộ trong component. Logic
...
- **(Update 17)** Kéo thả (drag-and-drop) là một tính năng thuần frontend — không cần thay đổi API
  backend, vì kết quả sắp xếp (DisplayOrder mới cho các step cùng cấp) được lưu qua đúng luồng Save
  hiện có (`UpdateEutrTemplatesUseCase`/`flattenForSave` của `useStepTree`) giống hệt khi dùng nút
  Move Up/Move Down — chỉ khác cử chỉ kích hoạt (kéo thả thay vì bấm nút), tái sử dụng nguyên vẹn
  hàm `reorderSiblings` đã có trong `useStepTree.js`. Việc chọn thư viện/kỹ thuật kéo thả cụ thể
  (native HTML5 drag events hay một thư viện DnD) sẽ được chốt ở bước `/speckit-plan`.
- **(Update 17)** Vì phạm vi kéo thả giới hạn "chỉ đổi thứ tự cùng cấp" (theo lựa chọn đã xác nhận),
  tính năng này không cần logic validate chống vòng lặp cha-con (cycle detection) mà việc reparent
  qua kéo thả sẽ cần đến — giữ độ phức tạp implementation tương đương với Move Up/Move Down hiện có.
- **(Update 19)** Bảng `eutr_reference_types` và API `GET /api/eutr-reference-types` đã tồn tại sẵn
  từ feature 006-eutr-reference-types (đã triển khai, có CRUD riêng và màn hình quản lý riêng) —
  đợt cập nhật này của 003-eutr-templates chỉ tái sử dụng API đọc sẵn có (`GET
  /api/eutr-reference-types`, trả `IEnumerable<EutrReferenceTypes>`), KHÔNG cần thêm backend mới
  cho việc này. Các giá trị Id hiện đang được seed sẵn (khớp với 5 giá trị PO/Vendor/Invoice/
  Delivery note/General agreement trước đây dùng làm TAKE_FROM_OPTIONS cứng) được coi là dữ liệu
  đã tồn tại trong bảng `eutr_reference_types`; không cần migrate/seed lại dữ liệu TakeFrom đã lưu
  trên các `eutr_template_details` hiện có vì Id vẫn giữ nguyên ý nghĩa.
- **(Update 19)** `DataGridStyled` (`compliance-client/src/presentation/components/shared/DataGridStyles.jsx`)
  là component styling dùng chung đã có sẵn, chỉ bọc quanh MUI `DataGrid` để áp style/height nhất
  quán — không cần tạo component mới; TemplateListPage tái sử dụng nguyên component này theo đúng
  pattern các trang danh sách EUTR khác (`eutr-reference-types`, `eutr-steps`, `eutr-documents`, …).
- **(Update 19)** Việc đổi TableContainer sang DataGridStyled không làm thay đổi hợp đồng API danh
  sách hiện có (`useEutrTemplatesData`, `GetEutrTemplatesUseCase`) — vẫn dùng chung
  `paginationModel`/`setPaginationModel`/`filterModel`/`setFilterModel`/`total`/`data`/`loading`
  đã có, chỉ đổi cách các state này được truyền vào `DataGrid` (`paginationModel`,
  `onPaginationModelChange`, `rowCount`, `loading`) thay vì vào `TablePagination`/JSX Table thủ công.
- **(Update 20)** `useEutrTemplatesData` đã sẵn `filterModel`/`setFilterModel` (Update 11) nên việc
  bật filter theo cột không cần hook mới — chỉ cần mở rộng whitelist `FilterMap` ở backend
  (`EutrTemplatesRepository.cs`) và merge 2 nguồn ghi vào `filterModel` (ô tìm kiếm nhanh + panel
