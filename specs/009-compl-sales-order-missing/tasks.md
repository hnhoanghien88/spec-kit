...
12. Polish → confirm no regression to the existing `test-alert` flow, no remaining `DeleteBySalesIdAsync` reference, the alert email's columns render correctly (14, Sales order last), the Excel attachment matches the email body (including the Expired-row highlight, the combined Sales order column, and the header styling), duplicate Master code/Code/Type/Product rows collapse to one with their sales order codes combined, and a known no-delivery-date sales order (e.g. SO006831) no longer appears in `compl_so_missing` when the screen shows it as not missing
