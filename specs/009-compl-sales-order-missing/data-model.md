...
## Diagnostic without-Sales-order column selection (2026-08-31, display-only, not persisted)

Per spec.md FR-023/FR-024, a second alert trigger presents the same 13 non-Sales-order columns and the same computed values (Status, Days remaining, dedup grouping, Expired highlighting, header styling — all unchanged) as the existing trigger, but the Sales order column is entirely absent from both the email body and the Excel attachment it sends (research.md R16). This introduces no new entity field, no new DTO property, and no new persisted state: `ComplSoMissing`, `ComplSoMissingResponseDto` (including `CombinedSalesIds`), the repository, and `compl_so_missing` itself are all unchanged and identical to what the existing trigger already reads and writes. The only difference is which entry of the `customHeaders` dictionary is populated at the call site that builds this trigger's own alert content — a projection-time, per-call decision, not a data-model change. Per spec.md FR-025, the same per-call decision also selects the alert's title string ("Missing compliance Information" instead of "Sales orders with missing compliance") — also display-only, not persisted anywhere.

## Per-trigger recipient-group exclusion (2026-09-03, call-time only, not persisted)

Per spec.md FR-026/FR-027/FR-028, the two Recipient Groups already resolved per current record — Responsible emails (from each record's `RespGroups`, parsed from `ResponsibleGroupsJson`) and Alert emails (from each record's `AlertGroups`, parsed from `AlertGroupsJson`) — are unchanged in how they are resolved (research.md R4), but each trigger now excludes one of the two groups from its own send: the existing test-sales-order-alert trigger excludes Alert emails, the diagnostic test-sales-order-alert-without-salesId trigger excludes Responsible emails (research.md R17). This introduces no new entity field, no new DTO property, and no new persisted state — `ComplSoMissing`, `ComplSoMissingResponseDto`, `AlertGroupsJson`/`ResponsibleGroupsJson`, the repository, and `compl_so_missing` itself are all unchanged. The only difference is two boolean call-site arguments (`includeResponsibleEmails`, `includeAlertEmails`) passed into `SendMailAndNotificationForSalesOrderMissing`, which control whether the already-resolved `responsibleEmails`/`alertEmails` lists are folded into that call's `allEmailList`/`ccEmailList` — a call-time decision affecting only who the alert is addressed to (email To/Cc and in-app notification recipients), not any stored or displayed data. The alert content itself — including the Responsible emails *column* shown in the email/Excel table, which is a separate, always-included display value sourced per-record (data-model.md "Display-only computed values") — is unaffected by which group the send is addressed to.
