...
- **2026-09-07 update**: User Story 7 (FR-021–FR-024, SC-011–SC-013) caps the same screen's "Status" percentage at 30% for a row whose BOM column reads "Missing", scaled by that row's existing saved applied/total compliance ratio. The formula (× 30 vs. × 100) and the field names it reads (`TotalApplied`, `TotalCompliances`, BOM status) are retained because they identify the existing saved values this display rule reads, not a new implementation choice. This update is display-only against already-saved data — confirmed no stored-procedure or schema change is needed, so no migration file accompanies this spec update (see Assumptions). Re-validated — all items still pass.
